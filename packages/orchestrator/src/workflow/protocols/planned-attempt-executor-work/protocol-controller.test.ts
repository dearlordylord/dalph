...
it.effect("does not retain a released protocol owner after an in-flight intent append", () =>
  Effect.gen(function* () {
    const controller = yield* makePlannedAttemptProtocolController()
    const reserved = yield* controller.reserve(correlation)
    expect(Option.isSome(reserved)).toBe(true)
    ...
    const appendEntered = yield* Deferred.make<void>()
    const allowAppend = yield* Deferred.make<void>()
    const intent = yield* reserved.value
      .commitIntent(
        Deferred.succeed(appendEntered, undefined).pipe(
          Effect.andThen(Deferred.await(allowAppend)),
          Effect.andThen(Effect.succeed("intent"))
        )
      )
      .pipe(Effect.forkChild)
    yield* Deferred.await(appendEntered)

    // The append is still in flight, so releasing the pre-intent owner must
    // win.  The post-append state check then observes no owner to update.
    yield* reserved.value.release
    yield* Deferred.succeed(allowAppend, undefined)
    expect(yield* Fiber.join(intent)).toBe("intent")
...
it.effect("waits for Terminal to finish before releasing a durable intent owner", () =>
  Effect.gen(function* () {
    const controller = yield* makePlannedAttemptProtocolController()
    const reserved = yield* controller.reserve(correlation)
    expect(Option.isSome(reserved)).toBe(true)
    ...
    // Let acquireTerminal reach its durable-intent wait without relying on a
    // wall-clock delay; the owner remains held until the explicit release.
...
it.effect("does not reuse a released permit for protocol facts or guarded work", () =>
  Effect.gen(function* () {
    const controller = yield* makePlannedAttemptProtocolController()
    const reserved = yield* controller.reserve(correlation)
    expect(Option.isSome(reserved)).toBe(true)
    ...
    yield* reserved.value.release
    expect(yield* reserved.value.activate).toBe("Released")
    expect(Exit.isFailure(yield* Effect.exit(reserved.value.commitIntent(Effect.succeed("stale-intent"))))).toBe(true)
    expect(Exit.isFailure(yield* Effect.exit(reserved.value.recordFact(Effect.succeed("stale-fact"))))).toBe(true)
    expect(
      Exit.isFailure(yield* Effect.exit(withPlannedAttemptProtocolPermit(reserved.value, correlation, Effect.void)))
    ).toBe(true)
