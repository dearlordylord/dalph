 ...
 */
export const PlannedAttemptExecutorCorrelation = Schema.Struct({
  attemptId: AttemptId,
  runId: RunId
})
...
const reportCorrelation = (
  report: PlannedAttemptExecutorReport
): PlannedAttemptExecutorCorrelation => report.correlation
...
export class PlannedAttemptExecutor extends Context.Service<
...
export const plannedAttemptExecutorCorrelationKey = (
  correlation: PlannedAttemptExecutorCorrelation
): string =>
  JSON.stringify({
 ...
 */
export const makeControlledFakePlannedAttemptExecutorLayer = (
  steps: ReadonlyArray<ControlledFakeExecutorStep>
) =>
  Layer.effect(
    PlannedAttemptExecutor,
    Effect.gen(function*() {
      const state = yield* Ref.make<ControlledFakeExecutorState>({
        remaining: steps,
        reports: new Map()
      })

      const consume = Effect.fn("ControlledFakeExecutor.consume")(
        function*(
          requestTag: ControlledFakeExecutorStep["_tag"],
          plannedAttempt: PlannedTaskAttemptType
        ) {
          const correlation = plannedAttemptExecutorCorrelation(plannedAttempt)
          const step = yield* Ref.modify(state, (current) =>
            [
              current.remaining[0],
              {
                ...current,
          ...
          if (
            step._tag !== requestTag
            || !sameCorrelation(step.correlation, correlation)
          ) {
            return yield* new ControlledFakeExecutorMismatch({
              detail: `expected ${step._tag} for ${
                plannedAttemptExecutorCorrelationKey(step.correlation)
          ...
          yield* Ref.update(state, (current) => ({
            ...
            reports: new Map([
              ...current.reports,
              [plannedAttemptExecutorCorrelationKey(correlation), step.report]
            ])
          }))
          return step.report
        }
          ...
          Ref.get(state).pipe(
            Effect.map((current) =>
              Option.fromUndefinedOr(
                current.reports.get(plannedAttemptExecutorCorrelationKey(correlation))
              )
            )
          ),
...
export const controlledFakePlannedAttemptExecutorLayer = Layer.effect(
  PlannedAttemptExecutor,
  Effect.gen(function*() {
    const reports = yield* Ref.make<
      ReadonlyMap<string, PlannedAttemptExecutorReport>
    >(new Map())
    const record = (
      plannedAttempt: PlannedTaskAttemptType,
      report: PlannedAttemptExecutorReport
    ...
    return PlannedAttemptExecutor.of({
      project: (correlation) =>
        Ref.get(reports).pipe(
          Effect.map((current) =>
            Option.fromUndefinedOr(
              current.get(plannedAttemptExecutorCorrelationKey(correlation))
            )
          )
        ),
