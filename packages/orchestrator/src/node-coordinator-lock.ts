...
const acquisitionUnavailable =
  ...
  (failure: unknown): CoordinatorLockUnavailable =>
    new CoordinatorLockUnavailable({ detail: failureDetail(failure), operation: "CoordinatorLock.acquire", target })
...
const sameFile = (left: FileSystem.File.Info, right: FileSystem.File.Info): boolean => {
  const leftInode = Option.getOrNull(left.ino)
  const rightInode = Option.getOrNull(right.ino)
  return leftInode !== null && rightInode !== null && left.dev === right.dev && leftInode === rightInode
}

const observeOwnership = Effect.fn("CoordinatorLock.Node.observeOwnership")(function* (
  fileSystem: FileSystem.FileSystem,
  directory: FileSystem.File,
  gitCommonDirectory: GitCommonDirectoryLocator
) {
  const [heldDirectory, observedPath] = yield* Effect.all([directory.stat, fileSystem.stat(gitCommonDirectory)]).pipe(
    Effect.mapError(() => new CoordinatorLockObservationContradiction({ gitCommonDirectory }))
  )
  if (!sameFile(heldDirectory, observedPath)) {
    return yield* new CoordinatorLockObservationContradiction({ gitCommonDirectory })
  }
})
...
export const nodeCoordinatorLockAdapterLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const acquire = Effect.fn("CoordinatorLock.Node.acquire")(function* (target: GitCommonDirectoryTarget) {
      ...
      const directory = yield* Effect.gen(function* () {
        const openedDirectory = yield* fileSystem
          .open(gitCommonDirectory, { flag: "r" })
          .pipe(Scope.provide(directoryScope), Effect.mapError(acquisitionUnavailable(target)))
        const directoryInfo = yield* openedDirectory.stat.pipe(Effect.mapError(acquisitionUnavailable(target)))
