...
import { projectTaskDagWire } from "./graph.js"
...
const dagSnapshotArbitrary = fc.integer({ min: 0, max: 8 }).chain((taskCount) => {
  ...
  return fc.array(fc.boolean(), { minLength: possibleEdgeCount, maxLength: possibleEdgeCount }).map((edges) => {
    let edgeIndex = 0
    return {
      revision: "bounded-property-v1",
      tasks: Array.from({ length: taskCount }, (_, dependantIndex) => {
        const prerequisiteIds: Array<string> = []
        for (let prerequisiteIndex = 0; prerequisiteIndex < dependantIndex; prerequisiteIndex++) {
          if (edges[edgeIndex] === true) {
            prerequisiteIds.push(`task-${prerequisiteIndex}`)
          }
          edgeIndex++
        }
        return { id: `task-${dependantIndex}`, lifecycle: open, parentTaskId: null, prerequisiteIds }
      })
    }
  })
})
...
it("preserves canonical construction and traversal laws across bounded DAGs", () => {
  fc.assert(
    fc.property(dagSnapshotArbitrary, (wire) => {
      ...
      const reversed = validSnapshot({ ...wire, tasks: [...wire.tasks].reverse() })
      const roundTripResult = projectTaskDagWire(JSON.parse(graph.canonicalJson()))
      expect(roundTripResult._tag).toBe("Valid")
      if (roundTripResult._tag === "Invalid") return
      const roundTripped = roundTripResult.snapshot
      const positions = new Map(graph.topologicalOrder().map((taskId, position) => [taskId, position]))
      ...
      for (const task of wire.tasks) {
        ...
        for (const prerequisite of task.prerequisiteIds) {
          expect(positions.get(TaskId.make(prerequisite))).toBeLessThan(dependantPosition)
        }
      }
    }),
    { numRuns: 100 }
