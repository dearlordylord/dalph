...
type CoordinatorLockAcquireError = CoordinatorLockHeld | CoordinatorLockUnavailable

export type CoordinatorOwnershipError = CoordinatorLockObservationContradiction | CoordinatorOwnershipLost
...
export class CoordinatorOwnership extends Context.Service<CoordinatorOwnership, CoordinatorOwnershipCapability>()(
...
interface CoordinatorLockService {
  ...
  ) => Effect.Effect<CoordinatorOwnershipCapability, CoordinatorLockAcquireError, Scope.Scope>
}
...
interface ControlledCoordinatorLockService {
  readonly contradict: (target: GitCommonDirectoryTarget) => Effect.Effect<void, CoordinatorLockUnavailable>
}
...
export const guardCoordinatorMutation = <A, E, R>(
  ...
  Effect.suspend(() =>
    Deferred.isDoneUnsafe(signal) ? Deferred.await(signal) : Effect.raceFirst(mutation, Deferred.await(signal))
  )

/** Resolves every raw path alias before an adapter observes ownership. */
export const resolveGitCommonDirectory = Effect.fn("CoordinatorLock.resolveGitCommonDirectory")(function* (
  ...
  const canonicalPath = yield* fileSystem
    ...
    .pipe(
      Effect.mapError(
        (failure) =>
          new CoordinatorLockUnavailable({ detail: String(failure), operation: "CoordinatorLock.acquire", target })
      )
    )
...
export const controlledCoordinatorLockLayer = Layer.effectContext(
  Effect.gen(function* () {
    ...
    const active = yield* Ref.make<ReadonlyMap<GitCommonDirectoryLocator, ControlledOwnershipState>>(new Map())

    const acquire = Effect.fn("CoordinatorLock.Controlled.acquire")(function* (target: GitCommonDirectoryTarget) {
      ...
      const acquired = yield* Ref.modify(active, (current) => {
        ...
        return [true, new Map([...current, [gitCommonDirectory, { signal, token }] as const])] as const
      })
      if (!acquired) {
    ...
    const contradict = Effect.fn("CoordinatorLock.Controlled.contradict")(function* (target: GitCommonDirectoryTarget) {
      const gitCommonDirectory = yield* resolveGitCommonDirectory(fileSystem, target)
      const ownership = yield* Ref.modify(active, (current) => {
        ...
        return [found, new Map([...current].filter(([key]) => key !== gitCommonDirectory))] as const
      })
      if (ownership !== undefined) {
        yield* Deferred.fail(ownership.signal, new CoordinatorLockObservationContradiction({ gitCommonDirectory }))
      }
    })

    const lock = CoordinatorLock.of({ acquire })
    const control = ControlledCoordinatorLock.of({ contradict })
    return Context.empty().pipe(Context.add(CoordinatorLock, lock), Context.add(ControlledCoordinatorLock, control))
  })
)
