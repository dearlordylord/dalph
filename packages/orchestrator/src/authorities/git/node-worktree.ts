...
import { type GitCommonDirectoryTarget } from "../coordinator-ownership/ownership.js"
...
const parseWorktreeRecords = Effect.fn("GitWorktree.Node.parseWorktreeRecords")(function* (
...
export const nodeGitWorktreeLayer = (gitDirectory: GitCommonDirectoryTarget) =>
  Layer.effect(
    GitWorktree,
    Effect.gen(function* () {
      ...
      const runRead = Effect.fn("GitWorktree.Node.runRead")(function* (
        ...
        return yield* commands
          ...
          .pipe(
            Effect.mapError(
              (failure) => new GitWorktreeReadFailure({ detail: failure.detail, worktree: plan.worktree })
            )
          )
      })
      ...
      const readPlannedWorktree = Effect.fn("GitWorktree.Node.readPlannedWorktree")(function* (
        ...
        const pathExists = yield* fileSystem
          .exists(plan.worktree)
          .pipe(
            Effect.mapError(
              (failure) => new GitWorktreeReadFailure({ detail: String(failure), worktree: plan.worktree })
            )
          )
        if (pathExists) return yield* new UntrackedWorktreePath({ worktree: plan.worktree })

        const branch = yield* runRead(plan, ["rev-parse", "--verify", "--quiet", `${plan.branch}^{commit}`])
        ...
        return PlannedBranchReady.make({ baseSha: plan.baseSha, branch: plan.branch, headSha })
      })

      const createPlannedWorktree = Effect.fn("GitWorktree.Node.createPlannedWorktree")(function* (
        plan: PlannedTaskAttempt
      ) {
        const branch = yield* commands
          .run(gitDirectory, ["rev-parse", "--verify", "--quiet", `${plan.branch}^{commit}`])
          .pipe(
            Effect.mapError(
              (failure) => new GitWorktreeCreateFailure({ detail: failure.detail, worktree: plan.worktree })
            )
          )
        const args =
          branch.exitCode === 0
