...
import { GitCommonDirectoryTarget } from "../coordinator-ownership/ownership.js"
...
const makePlan = (baseSha: GitCommitSha, branch: string, worktree: string) =>
...
const withRepository = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const directory = yield* fileSystem.makeTempDirectoryScoped({ prefix: "dalph-git-worktree-test-" })
    ...
    return yield* use({ baseSha, directory, gitDirectory: GitCommonDirectoryTarget.make(`${repository}/.git`) })
    ...
    Effect.scoped(
      withRepository(({ baseSha, directory, gitDirectory }) =>
        Effect.gen(function* () {
          ...
          expect(
            yield* FileSystem.FileSystem.pipe(Effect.flatMap((fileSystem) => fileSystem.exists(plan.worktree)))
          ).toBe(false)
        }).pipe(Effect.provide(adapterLayer(gitDirectory)))
      )
    )
  ...
  it.effect("decodes branch-only, detached, duplicate, and malformed Git observations", () =>
    Effect.scoped(
      withRepository(({ baseSha, directory, gitDirectory }) => {
        ...
        const readWith = (execute: (args: ReadonlyArray<string>) => Effect.Effect<GitCommandResult>) =>
          Effect.gen(function* () {
            const git = yield* GitWorktree
        ...
        return Effect.gen(function* () {
          ...
          for (const output of [
            ...
            const failure = yield* readWith(() => Effect.succeed(commandResult(0, output))).pipe(Effect.flip)
    ...
    Effect.scoped(
      withRepository(({ baseSha, directory, gitDirectory }) => {
        ...
        return Effect.gen(function* () {
          ...
          expect(
            yield* withScript(
              (args) => Effect.succeed(args[0] === "worktree" ? commandResult(0) : commandResult(0, "not-a-sha")),
              (git) => Effect.flip(git.readPlannedWorktree(plan))
            )
          ...
          expect(
            yield* withScript(
              (args) => Effect.succeed(args[0] === "worktree" ? commandResult(0) : commandResult(2)),
              (git) => Effect.flip(git.readPlannedWorktree(plan))
            )
          ).toBeInstanceOf(GitWorktreeReadFailure)
      ...
      withRepository(({ baseSha, directory, gitDirectory }) => {
          ...
          Layer.provide(
            Layer.succeed(
              ...
              FileSystem.makeNoop({
                exists: () =>
                  Effect.fail(
                    PlatformError.systemError({ _tag: "PermissionDenied", module: "GitWorktreeTest", method: "exists" })
                  )
              })
            )
          )
        ...
        const deniedSpawnerLayer = Layer.succeed(
          ...
          ChildProcessSpawner.make(() =>
            Effect.fail(
              PlatformError.systemError({ _tag: "PermissionDenied", module: "GitWorktreeTest", method: "spawn" })
            )
          )
        )
        ...
        return Effect.gen(function* () {
          ...
          }).pipe(Effect.provide(nodeGitCommandLayer), Effect.provide(deniedSpawnerLayer))
