  ...
  Effect.gen(function* () {
    ...
    const layer = journaledTestLayer(() =>
      Ref.updateAndGet(calls, (count) => count + 1).pipe(
        Effect.flatMap((count) =>
          count === 1
            ? Deferred.succeed(firstCallStarted, undefined).pipe(Effect.andThen(Effect.never))
            : Effect.succeed(
                AuthoritativePlannedAttemptWorktreeObserved.make({
                  observation: AttemptWorktreeLost.make({ plannedAttempt })
                })
    ...
    yield* runWithJournal(
      Effect.gen(function* () {
        ...
        const reopened = yield* makeApplicationExitLifecycle()
        const reopenedOwner = yield* reopened.admission.acquireForwardOwner("InterruptibleBoundary")
