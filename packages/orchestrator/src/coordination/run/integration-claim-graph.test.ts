...
it.effect("rereads the graph after checking an integration claim while retaining the exact target", () =>
  Effect.gen(function* () {
    ...
    for (const count of [64, 256]) {
      ...
      const cost = yield* Effect.gen(function* () {
        ...
        expect((yield* resources.snapshot).heldResponsibilityPositions.has(history.responsibility.queuedAt)).toBe(true)
        expect(
