...
const currentProjectionJournal = (
  ...
  const records =
    includeRunBeginning && reconstructed.workflowHistory.records[0]?.event._tag !== "WorkflowRunBegan"
      ? [began, ...reconstructed.workflowHistory.records]
      : reconstructed.workflowHistory.records
  const prefix = reconstructed.workflowHistory.prefix ?? acceptedJournalPrefixFromValidatedHistory(runId, records)
  const state = {
    ...
    reconstructed: { ...reconstructed, workflowHistory: { records, prefix } },
    prefix
  }
  const journal = Journal.of({
    ...inRunJournal,
    readAccepted: () => Effect.succeed(prefix),
    state: currentSignalOf(state)
  })
  return Layer.merge(Layer.succeed(InRunJournal, inRunJournal), Layer.succeed(Journal, journal))
}
    ...
    Effect.gen(function* () {
      ...
      for (const graphAfterDirection of [true, false]) {
        ...
        const afterObservationRecovery = yield* makeRunRecoveryProjection(
          ...
          Effect.provide(currentProjectionJournal(coverageRunId, coverageTarget, afterObservation))
        )
        ...
        const graphRefreshRecovery = yield* makeRunRecoveryProjection(
          ...
          Effect.provide(currentProjectionJournal(coverageRunId, coverageTarget, afterLaterGraph))
        )
      ...
      const noDirectionRecovery = yield* makeRunRecoveryProjection(
        ...
        Effect.provide(currentProjectionJournal(coverageRunId, coverageTarget, noDirectionState))
      )
      expect(
      ...
      const graphlessRecovery = yield* makeRunRecoveryProjection(
        ...
        Effect.provide(currentProjectionJournal(coverageRunId, coverageTarget, graphlessState))
      )
...
it("does not seed a continuation graph read from a foreign immutable Run target", () => {
  ...
  return Effect.runPromise(
    Effect.gen(function* () {
      const resources = yield* makeIntegrationTargetResourceController()
      const recovery = yield* makeRunRecoveryProjection(coverageRunId, undefined, resources).pipe(
        Effect.provide(currentProjectionJournal(coverageRunId, coverageTarget, reconstructed))
      )
      const projection = yield* recovery.readDeliveryProjection
...
it("fails closed for a valid no-begin prefix with a paired pending Git read", () => {
  ...
  return Effect.runPromise(
    Effect.gen(function* () {
      const resources = yield* makeIntegrationTargetResourceController()
      const recovery = yield* makeRunRecoveryProjection(coverageRunId, undefined, resources).pipe(
        Effect.provide(currentProjectionJournal(coverageRunId, coverageTarget, reconstructed, false))
      )
      const projection = yield* recovery.readDeliveryProjection
...
effectIt.effect("uses normal-layer current state without exporting records and rejects a mismatched run", () =>
  Effect.gen(function* () {
    ...
    const mismatchedRecovery = yield* makeRunRecoveryProjection(coverageRunId).pipe(
      Effect.provide(mismatchedJournal)
    )
