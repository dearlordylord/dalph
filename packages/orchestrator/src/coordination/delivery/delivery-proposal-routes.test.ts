...
describe("delivery proposal route matrix", () => {
  ...
  effectIt.effect("materializes executor-progress coverage from the route's exact pending task subjects", () =>
    Effect.gen(function* () {
      ...
      const proposal = trackerGraphReadProposalOf({
        ...
        requirement: {
          ...
          pendingReports: [
            ...
            { acceptedAt: JournalPosition.make(4), correlation: correlationA, taskId: taskA }
          ],
          runId,
          target,
          unresolvedReadOperationId: null
      ...
      const projected = projectTrackerSnapshot({ revision: "progress-route-coverage", tasks: [] })
      ...
      yield* executeFreshTrackerGraphRead(
        { _tag: "FreshOperationAction", operationId: OperationId.make("progress-route-read"), proposal },
        proposal.route,
        inertLease
      ).pipe(
        Effect.provideService(
