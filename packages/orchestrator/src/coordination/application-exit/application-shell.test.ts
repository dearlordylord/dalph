...
it.effect("surfaces a settled executor diagnostic through the final drain await", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const diagnostic = ApplicationExitDiagnostic.make("the final executor drain await retained this failure")
      const shell = yield* makeApplicationExitShell(defaultOwnership, { requestEnd: () => Effect.void })
      ...
      yield* shell.registerExecutorDrain({
        suspendExecutingExecutorWork: Deferred.succeed(initialStarted, undefined).pipe(
          Effect.andThen(Deferred.await(releaseInitial)),
          Effect.as([])
        )
      })

      const finalAwait = yield* shell.awaitExecutorDrains.pipe(Effect.exit, Effect.forkChild)
      const exiting = yield* shell.requestBoundary.requestExit.pipe(Effect.forkChild)
      yield* Deferred.await(initialStarted)
      // The first drain is still running, so this registration is started by
      // the driver but is absent from its initial result snapshot. Once the
      // first drain settles, the public final await can collect this second
      // result and must surface its diagnostic.
      yield* shell.registerExecutorDrain({
        suspendExecutingExecutorWork: Effect.fail(new ApplicationExitDrainFailure({ diagnostics: [diagnostic] }))
      })
      yield* Deferred.succeed(releaseInitial, undefined)
      const awaited = yield* Fiber.join(finalAwait)

      expect(awaited._tag).toBe("Failure")
      if (Exit.isFailure(awaited)) {
        const failure = Cause.findErrorOption(awaited.cause)
