...
export interface WorkflowRecoveryBoundaryObserver {
  readonly afterTaskWorkSessionIntentCommitted: (
    operation: typeof WorkflowOperation.cases.EstablishTaskWorkSession.Type
  ) => Effect.Effect<void>
}

const silentWorkflowRecoveryBoundaryObserver: WorkflowRecoveryBoundaryObserver = {
  afterTaskWorkSessionIntentCommitted: () => Effect.void
}
...
export const journaledWorkflowInterpreterLayer = <E, R>(
  ...
  boundaryObserver: WorkflowRecoveryBoundaryObserver = silentWorkflowRecoveryBoundaryObserver
) =>
  Layer.effect(
    WorkflowInterpreter,
    Effect.gen(function*() {
      ...
      const establishTaskWorkSession = Effect.fn(
        ...
        if (!hasIntent) {
          yield* boundaryObserver.afterTaskWorkSessionIntentCommitted(operation)
        }
