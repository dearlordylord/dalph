...
| One activation coordinator coalesces order-free signals, excludes a live owner before asking the controller to reserve capacity, starts one scoped runner, and rederives without waiting for that runner. Before intent the controller correlates the position with `SelectedTransitionIdentity`; after intent it correlates only with `OperationId`. A runtime-observed pre-intent exit frees the exact position, while a post-intent exit retains it until fresh provider evidence. | `deriveActivationPass`; `excludeOwnedTransitions`; `reserveTaskAdmissionPosition`; `claimActivationOwnership`; `recordOwnedOperationIntent`; `interruptBeforeOwnership`; `interruptAfterOwnershipBeforeIntent`; `interruptAfterIntent`; `recordOwnedResultAndRelease`; `observeCapacityConsumed`; `observeCapacityReleased`; all activation invariants and weakened counterexamples | deterministic coordinator cut points and generated controller sequences in `activation-coordinator.test.ts` and `activation-coordinator.property.test.ts`; the generated `activationOwnedThenDerivedPrefixStep` Quint-connect lane compares the complete activation projection after every action; readable positive and negative Quint profiles; the version-five closed action decoder |
...
- The tracker-mutation graph-fact lane is not applicable to the current
  ...
  same change.
- Issue #62 records authenticated run/task pause and resume commands without
  ...
  Issues #134 and #135 must extend M2, the adapter state comparison, and every
  applicable reopening lane when they consume these commands into run/task
  pause workflow semantics.
