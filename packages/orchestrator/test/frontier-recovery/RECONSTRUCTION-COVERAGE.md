...
| One activation coordinator coalesces order-free signals, excludes a live owner before asking the controller to reserve capacity, starts one scoped runner, and rederives without waiting for that runner. Before intent the controller correlates the position with `SelectedTransitionIdentity`; after intent it correlates only with `OperationId`. Provider evidence cannot attach before that operation identity exists. A runtime-observed pre-intent exit frees the exact position, while a post-intent exit retains it until fresh provider evidence. One task keeps one position when the provider reports a different `OperationId`; for example, at capacity two, conflicted A counts once and C can reserve the second position. | `deriveActivationPass`; `excludeOwnedTransitions`; `reserveTaskAdmissionPosition`; `claimActivationOwnership`; `recordOwnedOperationIntent`; `interruptBeforeOwnership`; `interruptAfterOwnershipBeforeIntent`; `interruptAfterIntent`; `recordOwnedResultAndRelease`; `observeCapacityConsumed`; `observeConflictingCapacityCorrelation`; `observeConflictingOperationReleased`; `observeCapacityReleased`; `observeCapacityAbsent`; `observeCapacityInterrupted`; `observeCapacityUnknown`; `readProviderInvocationForReconstruction`; `validateCurrentCapacityHistoryBeforeReconstruction`; `capacityUsageCountsTasksNotOperationCorrelations`; `correlationConflictRetainsOneTaskPosition`; `rejectedCapacityHistoryDerivesNoFrontier`; all activation invariants and weakened counterexamples | deterministic coordinator cut points and generated controller sequences in `activation-coordinator.test.ts` and `activation-coordinator.property.test.ts`; `mismatchedProviderOperationCountsTaskOnceAndAdmitsAnotherTaskTest`; `differentlyCorrelatedTerminalReportKeepsExpectedOperationHeldTest`; `repeatedMismatchedReportsKeepOneTaskPositionTest`; `restartRecreatesExactCorrelationConflictTest`; `unknownProviderReportKeepsExpectedTaskPositionTest`; `unknownReportDoesNotEraseDifferentlyCorrelatedActiveEvidenceTest`; `absentProviderReportReleasesExpectedTaskPositionTest`; `interruptedProviderReportReleasesExpectedTaskPositionTest`; `rejectsTwoCurrentCapacityOperationsForOneTaskTest`; `unresolvedTaskUsesTheOnlyPositionOnceTest`; focused exhaustive `capacityCorrelationProfileStep`; the generated crash/reconstruction Quint-connect lane includes a fresh provider read; the generated `activationOwnedThenDerivedPrefixStep` lane compares the complete activation projection after every action; readable positive and negative Quint profiles; the version-five closed action decoder. The task-correlation TypeScript tests are the required first step of the reopened #131 implementation. |
| The sampled driver never assigns production state or derives another scheduler. It appends normalized controls, rereads the journal, invokes `reduceManagedHistory`, and compares the model-owned normalized graph evidence, workflow event tags, responsibility identities, pause projection, transition identities and tags, operation identities, explanations, configured capacity, occupied tasks, controller reservations, activation owners, runners, provider observations, interruption/result markers, and pending trigger after every supported action. No comparator constructs or writes an expected production state. Separate readable tests assert the exact raw journal, graph-knowledge, responsibility, and pause shapes. | `frontierRecoveryCapacityOne`; `frontierRecoveryCapacityTwo`; `initCapacityOneResponsibilityFirstProfile`; direct and selected claim intent; `crash`; `restart`; the three named read profiles; `activationOwnedThenDerivedPrefixStep` | ten `frontier-recovery-reconstruction.mbt.test.ts` lanes (`S`), plus exact production projections in `frontier-recovery-reconstruction.test.ts` |
