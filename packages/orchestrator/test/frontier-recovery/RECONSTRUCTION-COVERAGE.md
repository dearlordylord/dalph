...
All executor invocation identities used by Model 2 capacity positions and
reports refer to the one opaque outer invocation that Dalph started for a
task. The executor's implementation, evidence, review, handback, retry,
restoration, and convergence operations use private `OperationId` values that
are absent from this generic model. Generic capacity uses only
...
| The task tracker returns a normalized target-closure membership read carrying its read shape, explicit coverage, completeness, potentially mixed-time consistency, boundary freshness, fact family, operation identity, revision, returned tasks, and proven absence; a predecessor identity alone does not add coverage. | `taskTrackerReturnsTargetClosureReadWithExplicitAbsenceCoverage`; `taskTrackerReturnsTargetClosureReadWithPredecessor`; `taskTrackerReturnsTargetClosureReadAtNextRevision` | three named Quint-connect `S` lanes, three sampled model witnesses, plus readable scenarios in `frontier-recovery-reconstruction.test.ts` |
...
| One activation coordinator coalesces order-free signals, excludes a live owner before asking the controller to reserve capacity, starts one scoped runner, and rederives without waiting for that runner. Before outer-invocation intent the controller keys the temporary position by `SelectedTransitionIdentity`; after intent it binds the position to `ExecutorOuterInvocationId`. An executor report cannot attach before that outer identity exists. A runtime-observed pre-intent exit frees the exact position, while a post-intent exit retains it until a fresh executor report. One task keeps one position when the executor reports a different outer invocation; for example, at capacity two, mismatched A counts once and C can reserve the second position. | `deriveActivationPass`; `excludeOwnedTransitions`; `reserveTaskAdmissionPosition`; `claimActivationOwnership`; `recordOwnedOperationIntent`; `interruptBeforeOwnership`; `interruptAfterOwnershipBeforeIntent`; `interruptAfterIntent`; `recordOwnedResultAndRelease`; `observeCapacityConsumed`; `observeConflictingCapacityCorrelation`; `observeReportedInvocationReleased`; `observeCapacityReleased`; `observeCapacityAbsent`; `observeCapacityInterrupted`; `observeCapacityUnknown`; `readExecutorInvocationForReconstruction`; `validateCurrentCapacityHistoryBeforeReconstruction`; `capacityUsageCountsTasksNotInvocationMismatches`; `executorInvocationMismatchRetainsOneTaskPosition`; `rejectedCapacityHistoryDerivesNoFrontier`; all activation invariants and weakened counterexamples | deterministic coordinator cut points and generated controller sequences in `activation-coordinator.test.ts` and `activation-coordinator.property.test.ts`; `mismatchedExecutorInvocationCountsTaskOnceAndAdmitsAnotherTaskTest`; `differentlyCorrelatedTerminalReportKeepsExpectedInvocationHeldTest`; `repeatedMismatchedReportsKeepOneTaskPositionTest`; `restartRecreatesExactExecutorInvocationMismatchTest`; `unknownExecutorReportKeepsExpectedTaskPositionTest`; `unknownReportDoesNotEraseDifferentlyCorrelatedActiveEvidenceTest`; `absentExecutorReportReleasesExpectedTaskPositionTest`; `interruptedExecutorReportReleasesExpectedTaskPositionTest`; `rejectsTwoUnfinishedOuterInvocationsForOneTaskTest`; `unresolvedTaskUsesTheOnlyPositionOnceTest`; focused exhaustive `executorInvocationMismatchReportProfileStep` and `executorInvocationMismatchReconstructionProfileStep`; the generated crash/reconstruction Quint-connect lane includes a fresh executor read; the generated `activationOwnedThenDerivedPrefixStep` lane compares the complete activation projection after every action; readable positive and negative Quint profiles; the version-five closed action decoder. The task-correlation TypeScript tests are the required first step of the reopened #131 implementation. |
| The sampled driver never assigns production state or derives another scheduler. It appends normalized controls, rereads the journal, invokes `reduceManagedHistory`, and compares the model-owned normalized graph evidence, workflow event tags, responsibility identities, pause projection, transition identities and tags, operation identities, explanations, configured capacity, occupied tasks, controller reservations, activation owners, runners, normalized executor reports, interruption/result markers, and pending trigger after every supported action. No comparator constructs or writes an expected production state. Separate readable tests assert the exact raw journal, graph-knowledge, responsibility, and pause shapes. | `frontierRecoveryCapacityOne`; `frontierRecoveryCapacityTwo`; `initCapacityOneResponsibilityFirstProfile`; direct and selected claim intent; `crash`; `restart`; the three named read profiles; `activationOwnedThenDerivedPrefixStep` | ten `frontier-recovery-reconstruction.mbt.test.ts` lanes (`S`), plus exact production projections in `frontier-recovery-reconstruction.test.ts` |
