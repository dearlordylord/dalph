...
export const isControlledDs02StageSequence = (stages: ReadonlyArray<string>): boolean =>
...
export const isControlledDs02ActionInventory = (
  actions: ReadonlyArray<{ readonly stage: string; readonly taskId: string }>
): boolean =>
  actions.every(({ taskId }) => ["A", "B", "C"].includes(taskId)) &&
  ["A", "B", "C"].every((taskId) =>
    isControlledDs02StageSequence(actions.filter((action) => action.taskId === taskId).map(({ stage }) => stage))
  )
...
export const isControlledDs02PassiveAction = (
  capture: ControlledDs02PassiveActionCapture,
...
export const isControlledDs02PassiveRead = (
  capture: ControlledDs02PassiveReadCapture,
