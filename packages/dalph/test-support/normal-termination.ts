...
import { makeNormalTerminationSettledA, type NormalTerminationSettledA } from "./settled-a.js"

/** Actual tracker calls, correlated with the pending durable complete-read intent. */
export interface NormalTerminationGraphRead {
...
export type NormalTerminationClaimCall =
...
export type NormalTerminationCut = "FinalGraphObserved" | "TerminationAppended"
export type NormalTerminationDeliveryHold = "CompletionClaimReplaced" | "CompletionClaimDeletionIntended"
const allTaskNames = ["A", "B", "C", "D", "E", "F", "G"] as const

type NormalTerminationCutState =
  | { readonly _tag: "Disabled" }
  | { readonly _tag: "Armed"; readonly at: NormalTerminationCut }
type NormalTerminationDeliveryHoldState =
  | { readonly _tag: "Disabled" }
  | { readonly _tag: "Armed"; readonly at: NormalTerminationDeliveryHold }

export interface NormalTermination extends SixTaskDeliveryRuntime {
  readonly facts: ReturnType<typeof makeSixTaskDeliveryFacts>
  readonly settledA: NormalTerminationSettledA
  readonly graphReads: Ref.Ref<ReadonlyArray<NormalTerminationGraphRead>>
  readonly specificationReads: Ref.Ref<ReadonlyArray<{ readonly target: TrackerTarget; readonly taskId: TaskId }>>
  readonly claimCalls: Ref.Ref<ReadonlyArray<NormalTerminationClaimCall>>
  readonly events: Queue.Queue<WorkflowEvent>
  readonly reached: Queue.Queue<NormalTerminationCut>
  readonly terminationCut: Ref.Ref<NormalTerminationCutState>
  readonly deliveryHold: Ref.Ref<NormalTerminationDeliveryHoldState>
  readonly heldDelivery: Queue.Queue<NormalTerminationDeliveryHold>
