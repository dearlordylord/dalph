...
import type { ControlledDs04TimerCheckpointInput } from "./controlled-ds04.js"
import type { ControlledDs03BoundarySnapshot } from "./controlled-characterization-types.js"
import { controlledDeliveryCharacterization as scenario } from "./controlled-characterization-catalog.js"

export interface ActiveGraphRefreshRefreshResult {
...
export const makeActiveGraphRefreshGraphRefresh: (
  reader: TrackerGraphReader["Service"],
  tracker: TestTrackerGraphReader["Service"]
) => Effect.Effect<ActiveGraphRefreshGraphRefresh> = Effect.fn("ActiveGraphRefresh.makeGraphRefresh")(function* (
  reader: TrackerGraphReader["Service"],
  tracker: TestTrackerGraphReader["Service"]
) {
  ...
  const result = yield* Deferred.make<ActiveGraphRefreshRefreshResult>()
  ...
  const run = <E, R>(input: ControlledDs04TimerCheckpointInput<E, R>) =>
    Effect.gen(function* () {
...
interface ActiveGraphRefreshGraphRefreshOperations {
...
export type ActiveGraphRefreshGraphRefresh = ActiveGraphRefreshGraphRefreshOperations & {
  readonly result: Deferred.Deferred<ActiveGraphRefreshRefreshResult>
}
