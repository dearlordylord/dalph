...
export type DistinctFinalityCut =
  | "AcceptedResult"
  | "IntegratorResult"
  | "Promotion"
  | "PromotionIntent"
...
export type DistinctFinalityFinalityCall =
...
type DistinctFinalityCutState =
  | { readonly _tag: "Disabled" }
  | { readonly _tag: "Armed"; readonly at: DistinctFinalityCut; readonly attemptId: AttemptId }

interface DistinctFinality extends SixTaskDeliveryRuntime {
  readonly facts: ReturnType<typeof makeSixTaskDeliveryFacts>
  readonly calls: Ref.Ref<ReadonlyArray<DistinctFinalityFinalityCall>>
  readonly finalityCut: Ref.Ref<DistinctFinalityCutState>
  readonly reached: Queue.Queue<
    { readonly _tag: "Crash"; readonly at: DistinctFinalityCut } | { readonly _tag: "Finished" }
  >
  readonly events: Queue.Queue<WorkflowEvent>
}

export const makeDistinctFinality = Effect.fn("DistinctFinality.makeDistinctFinality")(function* (): Effect.fn.Return<
  DistinctFinality,
  never,
  Crypto.Crypto | Scope.Scope
> {
  const facts = makeSixTaskDeliveryFacts("issue-277")
  const calls = yield* Ref.make<ReadonlyArray<DistinctFinalityFinalityCall>>([])
  ...
  const record = (call: DistinctFinalityFinalityCall) => Ref.update(calls, (all) => [...all, call])
  const interruptAt = (at: DistinctFinalityCut, attemptId: AttemptId) =>
    Effect.gen(function* () {
  ...
  const runtime = yield* makeSixTaskDeliveryRuntime(facts, {
    beforeAppend: (event) =>
      event._tag === "CompletionTaskAcknowledged"
        ...
        : Effect.void,
    afterAppend,
    makeFinality: Effect.fn("DistinctFinality.makeObservedFinality")(function* (tracker: TrackerMutation["Service"]) {
