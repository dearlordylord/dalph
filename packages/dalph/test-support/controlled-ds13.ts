...
import { isControlledExactB1Plan } from "./controlled-ds06.js"
...
const exactBPlanRecords = (records: ReadonlyArray<JournalRecord>) =>
  records.filter(
    ({ event, runId }) =>
      ...
      isControlledExactB1Plan(event.operation.plannedAttempt)
  )
...
const exactReadyBPublicationEvidence = (
  ...
  return (
    ...
    isControlledExactB1Plan(responsibility.plannedAttempt) &&
...
const hasExactFinalPositions = (publication: DeliveryRelationInputBundle) => {
  ...
  const expected = [
    { attemptId: scenario.attempts.B1, plan: isControlledExactB1Plan, taskId: scenario.taskIds.B },
...
const hasSingleExactBPlan = (records: ReadonlyArray<JournalRecord>) => {
  ...
  return (
    ...
    isControlledExactB1Plan(plan.event.operation.plannedAttempt)
  )
}
...
export const isControlledDs13CompleteCheckpoint = (
  publication: DeliveryRelationInputBundle,
  records: ReadonlyArray<JournalRecord>
) => {
