...
import { suspendCommandOrdinal } from "./controlled-characterization-fixture.js"
...
export const isControlledDs04CompleteCheckpoint = (
  ...
  return (
    isControlledDs04CheckpointPublication(publication) &&
...
export interface ControlledDs04TimerCheckpointInput<E, R> {
  ...
  readonly snapshot: Effect.Effect<ControlledDs03BoundarySnapshot, E, R>
  readonly startupDecision: RunFinalityDecision
}

interface ControlledDs04LiveOwnerContinuation<A, E, R> {
  readonly awaitResult: Effect.Effect<A, E, R>
  readonly begin: Effect.Effect<void, never, R>
}

export interface ControlledDs04TimerCheckpointResult<A> {
  readonly checkpoint: ControlledDs04Characterization
...
export const runControlledDs04TimerCheckpoint = <
  A = never,
  E = never,
  R = never,
  EContinuation = never,
  RContinuation = never
>(
  input: ControlledDs04TimerCheckpointInput<E, R>,
  continuation?: ControlledDs04LiveOwnerContinuation<A, EContinuation, RContinuation>
) =>
  Effect.gen(function* () {
    ...
    const awaitCompleteCheckpoint = (): Effect.Effect<void, E, R> =>
      input.nextPublication.pipe(
        ...
        Effect.flatMap(({ publication, records }) => {
          ...
          return isControlledDs04CompleteCheckpoint(publication, newRecords, input.attemptId)
            ? Effect.void
    ...
    } satisfies ControlledDs04TimerCheckpointResult<A>
  })
