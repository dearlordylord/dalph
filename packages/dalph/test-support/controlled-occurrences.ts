...
export type ControlledOccurrenceSource =
  | "Action"
  | "Control"
  | "Executor"
  | "Git"
...
export interface ControlledObservedOccurrence {
  readonly detail: string
  readonly kind: string
  readonly ordinal: number
  readonly source: ControlledOccurrenceSource
  readonly sourceSequence: number
}

/** One finer ordering point used to prove a required happens-before edge. */
export interface ControlledCausalLandmark extends ControlledObservedOccurrence {
  readonly key: string
}

interface ControlledOccurrenceInput {
  readonly detail?: string
  readonly kind: string
  readonly source: ControlledOccurrenceSource
}

export interface ControlledOccurrenceEvidence {
  readonly observedOccurrences: ReadonlyArray<ControlledObservedOccurrence>
}
...
export interface ControlledOccurrenceRecorder {
  readonly record: (input: ControlledOccurrenceInput) => Effect.Effect<void>
  readonly snapshot: Effect.Effect<ControlledOccurrenceEvidence>
}

/** Makes one shared monotonic recorder; Ref.modify keeps cross-fiber stamps atomic. */
export const makeControlledOccurrenceRecorder = Effect.gen(function* () {
  interface RecorderNode {
    readonly entry: ControlledObservedOccurrence
    readonly previous: RecorderNode | undefined
  }
  interface RecorderState {
    readonly count: number
    readonly head: RecorderNode | undefined
    readonly sourceSequences: ReadonlyMap<ControlledOccurrenceSource, number>
  }
  const occurrenceLimit = 20_000
  const state = yield* Ref.make<RecorderState>({ count: 0, head: undefined, sourceSequences: new Map() })
  const record = (input: ControlledOccurrenceInput) =>
    Ref.modify(state, (current) => {
      ...
      Effect.flatMap((accepted) =>
        accepted
          ? Effect.void
          : Effect.die(
              `controlled occurrence recorder exceeded ${occurrenceLimit} items at ${input.source}:${input.kind}`
            )
      )
    )
  const snapshot = Ref.get(state).pipe(
    Effect.map(({ head }) => {
      const reversed: Array<ControlledObservedOccurrence> = []
      let current = head
  ...
  return { record, snapshot } satisfies ControlledOccurrenceRecorder
})

export interface ControlledRequiredEdge {
...
export interface ControlledRequiredEdgeViolation {
  readonly edge: ControlledRequiredEdge
  readonly reason: "AfterNotAfterBefore" | "DuplicateEndpoint" | "MissingEndpoint"
}

const requiredOrderingClaimCount = 20

const occurrencesFor = (landmarks: ReadonlyArray<ControlledCausalLandmark>, key: string) =>
...
export const validateControlledRequiredEdges = (
  landmarks: ReadonlyArray<ControlledCausalLandmark>,
  edges: ReadonlyArray<ControlledRequiredEdge>
): ReadonlyArray<ControlledRequiredEdgeViolation> =>
  edges.flatMap<ControlledRequiredEdgeViolation>((edge) => {
...
export const reverseControlledRequiredEdge = (
  landmarks: ReadonlyArray<ControlledCausalLandmark>,
  edge: ControlledRequiredEdge
): ReadonlyArray<ControlledCausalLandmark> => {
...
export const controlledRequiredClaimCoverageIsComplete = (edges: ReadonlyArray<ControlledRequiredEdge>): boolean => {
...
export const controlledExpectedOccurrenceCounts: Readonly<Record<string, number>> = {
  ...
  "Journal:JournalRecoveryReadCalled": 2,
  "Journal:JournalRecoveryReadReturned": 2,
  "Journal:RemotePublicationAdmissionObserved": 1,
  "Journal:RemotePublicationAdmissionReadIntended": 1,
  ...
  "Publication:DeliveryRuntimeObservationPublished": 381,
...
export const controlledOccurrenceEvidenceIsComplete = (evidence: ControlledOccurrenceEvidence): boolean => {
  ...
  if (new Set(ordinals).size !== ordinals.length || ordinals.some((ordinal, index) => ordinal !== index + 1))
    return false
  const latestSourceSequences = new Map<ControlledOccurrenceSource, number>()
  ...
  const expectedEntries = Object.entries(controlledExpectedOccurrenceCounts)
  return (
