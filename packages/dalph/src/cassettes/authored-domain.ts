...
const targetPromotionOriginsFor = (
  ...
  const origins = story.flatMap((item, integratorIndex) => {
    ...
    if (
      item.correlation.session.expectedTargetHead !== request.expectedTargetHead ||
      item.correlation.session.integrationTarget.repository !== request.integrationTarget.repository ||
      item.correlation.session.integrationTarget.ref !== request.integrationTarget.ref
    ) {
      return []
    }
      ...
      : [
          {
            ...
            integratorIndex,
            plannedAttempt: item.correlation.session.plannedAttempt,
            request: TargetPromotionGitRequest.make({
              candidateCommit: request.candidateCommit,
              expectedTargetHead: item.correlation.session.expectedTargetHead,
              integrationTarget: item.correlation.session.integrationTarget
            })
          }
        ]
  })
...
const targetPromotionWorkflowKey = (origin: { readonly correlation: IntegratorRunCorrelation }): string =>
  JSON.stringify([
    String(origin.correlation.session.sessionId),
    String(origin.correlation.session.candidateResource),
    String(origin.correlation.session.plannedAttempt.attemptId),
    String(origin.correlation.session.plannedAttempt.taskId),
    String(origin.correlation.session.queuedAt),
    String(origin.correlation.session.startedAt),
    String(origin.correlation.session.targetLineageObservedAt),
    String(origin.correlation.ordinal)
  ])
...
const exactIntegratorIndexForPromotedHold = (
  ...
  story.findIndex(
    (candidate, candidateIndex) =>
      ...
      candidate._tag === "IntegratorRequestReceived" &&
      candidate.correlation.session.plannedAttempt.taskId === hold.promotedTaskId &&
      candidate.correlation.session.plannedAttempt.attemptId === hold.promotedAttemptId
  )
