...
/** Harness-only atomic handoff from one ordinary append result to its rendezvous bookkeeping. */
export const appendThenCompleteExecutorReportRendezvous = <A, E, R>(
  prepareAndAppend: Effect.Effect<A, E, R>,
  complete: Effect.Effect<void>,
  fail: Effect.Effect<void>
): Effect.Effect<A, E, R> =>
  Effect.uninterruptibleMask((restore) =>
    restore(prepareAndAppend).pipe(Effect.onExit((exit) => (Exit.isSuccess(exit) ? complete : Effect.void)))
  ).pipe(Effect.onExit((exit) => (Exit.isFailure(exit) ? fail : Effect.void)))
...
type AuthoredAppendableJournalEvent = Exclude<
  JournalRecord["event"],
...
const runAuthoredScenarioCassetteWith = (request: {
  ...
  return Effect.scoped(
    ...
    Effect.gen(function* () {
      ...
      interface DurableExecutorReportRequirement {
        readonly attemptId: AttemptId
        readonly ordinal: number
        readonly reportShape: string
      }
      const executorReportShape = (report: { readonly _tag: string; readonly result?: { readonly _tag: string } }) =>
        report._tag === "Terminal" ? `Terminal:${report.result?._tag ?? "Unknown"}` : report._tag
      const { requirements: durableReportBeforeSpecificationRelease } = cassette.story.reduce<{
        readonly latestReport: DurableExecutorReportRequirement | undefined
        readonly ordinals: ReadonlyMap<AttemptId, number>
        readonly requirements: ReadonlyMap<number, DurableExecutorReportRequirement>
      }>(
        ({ latestReport, ordinals, requirements }, item, index) => {
          if (item._tag === "PlannedAttemptExecutorWorkReported") {
            const ordinal = (ordinals.get(item.report.attemptId) ?? 0) + 1
            return {
              latestReport: {
                attemptId: item.report.attemptId,
                ordinal,
                reportShape: executorReportShape(item.report)
              },
              ordinals: new Map([...ordinals, [item.report.attemptId, ordinal] as const]),
              requirements
            }
          }
      ...
      const batchReadySignalFor = (
        ...
        const member = gate.members.find(
          (candidate) =>
            candidate.attemptId === durableReport.attemptId &&
            candidate.ordinal === durableReport.ordinal &&
      ...
      const durableExecutorReportKey = (requirement: DurableExecutorReportRequirement): string =>
        `${runId}:${requirement.attemptId}:${requirement.ordinal}:${requirement.reportShape}`
      const awaitDurableExecutorReport = Effect.fn("AuthoredCassette.awaitDurableExecutorReport")(function* (
        requirement: DurableExecutorReportRequirement
      ) {
        const key = durableExecutorReportKey(requirement)
        if ((yield* SubscriptionRef.get(durableExecutorReports)).has(key)) return
        yield* SubscriptionRef.changes(durableExecutorReports).pipe(
          Stream.filter((reports) => reports.has(key)),
          Stream.take(1),
          Stream.runDrain
        )
      })
                ...
                Effect.tap(() =>
                  Effect.gen(function* () {
                    if (
                      ...
                      const observed = durableExecutorReportKey({
                        attemptId: event.report.correlation.attemptId,
                        ordinal: event.ordinal,
                        reportShape: executorReportShape(event.report)
                      })
      ...
      const failExecutorReportBatchOnProviderExit = Effect.fn("AuthoredCassette.failExecutorReportBatchOnProviderExit")(
        function* (
          plannedAttempt: PlannedTaskAttempt,
          request: ExecutorReportRequest,
          exit: Exit.Exit<PlannedAttemptExecutorReport, unknown>
        ) {
          if (Exit.isSuccess(exit)) return
          const release = yield* Ref.get(executorReportBatchGate).pipe(
            Effect.map((gate) => {
              if (gate._tag !== "Configured" && gate._tag !== "Armed") return undefined
              const member = gate.members.find(
                (candidate) =>
              ...
              return member === undefined ? undefined : gate.release
            })
          )
          if (release === undefined) return
          yield* failExecutorReportBatch(
            release,
            `executor-report provider failed before batch append for ${plannedAttempt.taskId}:${plannedAttempt.attemptId}:${request}`
          )
        }
      )
      ...
      const runtimeLayerFor = (activationOrdinal: AuthoredRunActivationOrdinalType) => {
        ...
        const batchedInRunJournalLayer = Layer.effect(
          ...
          Effect.gen(function* () {
            ...
            const appendRegisteredExecutorReport = Effect.fn("AuthoredCassette.appendRegisteredExecutorReport")(
              function* (
                registration: Extract<ExecutorReportBatchRegistration, { readonly _tag: "Registered" }>,
                appendRunId: RunId,
                key: JournalRecord["key"],
                event: AuthoredAppendableJournalEvent
              ) {
                return yield* appendThenCompleteExecutorReportRendezvous(
                  Effect.gen(function* () {
                    ...
                    if (!responsibilityIsExact) {
                      return yield* Effect.die(
                        `executor-report batch append has no exact responsibility: ${registration.taskId}:${registration.attemptId}`
                      )
                    }
                    return yield* journal.append(appendRunId, key, event)
                  }),
                  completeExecutorReportBatchAppend(registration.appendIdentity),
                  failExecutorReportBatch(
                    registration.release,
                    `executor-report batch append failed or was interrupted for ${registration.appendIdentity}`
                  )
                )
              }
            )
            return InRunJournal.of({
              ...
              append: (appendRunId, key, event) =>
                Effect.gen(function* () {
                  ...
                  return yield* registration._tag === "Registered"
                    ? appendRegisteredExecutorReport(registration, appendRunId, key, event)
                    : journal.append(appendRunId, key, event)
                })
            })
          })
        )
        const executorLayer = controlledExecutorLayer(
          ...
          failExecutorReportBatchOnProviderExit
      ...
      const withAuthoredOperatorDriver = <A, E, R>(program: Effect.Effect<A, E, R>) =>
        Effect.scoped(
          Effect.gen(function* () {
            ...
            const driveExecutorReportBatchGate = Effect.gen(function* () {
            ...
            const driveTaskWorkSpecificationReadBoundaryRelease = Effect.gen(function* () {
              ...
              if (durableReport !== undefined) {
                ...
                const memberReady = batchReadySignalFor(gate, durableReport, request)
