  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
            ...
            Ref.get(lifecycle).pipe(
              Effect.flatMap((currentLifecycle) => {
                const projection = projectTrackerSnapshot({
                  ...
                  rootTaskId: taskId,
                  tasks: [
                    { id: taskId, lifecycle: { _tag: currentLifecycle }, parentTaskId: null, prerequisiteIds: [] }
                  ]
                })
        ...
        if (!(yield* Ref.get(terminated))) return yield* Effect.die("hermetic MVP did not converge")
        ...
        expect((yield* git.runInWorktree(repository, ["show-ref", "--verify", "refs/heads/unrelated"])).exitCode).toBe(
          0
        )
