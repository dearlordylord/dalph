    ...
    const contradictory = {
      ...
      story: [
        ...singleton.story.slice(0, -1),
        {
          ...
          taskWork: {
            ...assertions.taskWork,
            absences: [{ _tag: "NoPlannedWorkUndertakenForTask", taskId: "A" }],
...
it.effect("derives failed task-work results and safely suspended orchestration evidence from recorded handling", () =>
  Effect.gen(function* () {
    const failed = {
      ...coordinatorProcessDeathContinuesAuthoredCassette,
      story: coordinatorProcessDeathContinuesAuthoredCassette.story.map((item) => {
        ...
        if (item._tag === "ExpectedBehavior") {
          return {
            ...item,
            orchestration:
              item.orchestration?.map((evidence) =>
                evidence._tag === "PlannedAttemptExecutorWorkReported" &&
                evidence.report === "ExecutorWorkTerminalCompleted"
                  ? { ...evidence, report: "ExecutorWorkTerminalFailed" as const }
                  : evidence
              ) ?? null,
  ...
  Effect.gen(function* () {
    const runOnce = Effect.gen(function* () {
      ...
      return yield* useAuthoredScenarioCassetteWithCrypto(
        maintainedAuthoredCassetteCatalog.targetPromotionSuccess,
        (promoted) => runIntegrationFinalityProtocolCassetteFromPromotedRecords(cassette, promoted.records)
