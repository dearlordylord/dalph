...
it.effect("publishes B F2 through one active refresh and rereads G1 after Safe before D begins", () =>
  Effect.gen(function* () {
    ...
    const authoredBSafeAt = story.findIndex(
      (item) =>
    ...
    expect(authoredBSafeAt).toBeGreaterThanOrEqual(0)
    expect(story.slice(authoredBSafeAt, authoredBSafeAt + 6)).toMatchObject([
      {
...
it.effect("records exactly one C2 Safe ordinal before Continue B", () =>
  Effect.gen(function* () {
    const run = yield* capstoneRun
    const safeCapture = run.observationCaptures.find(
      (capture) =>
    ...
    const continueCapture = run.observationCaptures.find(
      (capture) =>
        capture._tag === "AuthoredStoryOccurrenceCaptured" &&
    ...
    expect(safeCapture).toBeDefined()
    expect(continueCapture).toBeDefined()
    if (safeCapture === undefined || continueCapture === undefined) return
    expect(safeCapture.captureOrder).toBeLessThan(continueCapture.captureOrder)
  })
)
