...
import { makeDistinctFinality, type DistinctFinalityCut } from "../../test-support/distinct-finality.js"
import type { SixTaskCandidateRead } from "../../test-support/six-task-integrator-git.js"

type Fixture = Effect.Success<ReturnType<typeof makeDistinctFinality>>
...
const start = Effect.fn("DistinctFinalityTest.start")(function* (fixture: Fixture) {
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const fixture = yield* makeDistinctFinality()
...
const beforeD = Effect.fn("DistinctFinalityTest.beforeD")(function* () {
  const fixture = yield* makeDistinctFinality()
  const process = yield* start(fixture)
  yield* process.held(["B", "C", "D"])
...
for (const cut of [
  ...
  "ClaimDeletionIntent",
  "ClaimDeletion"
] satisfies ReadonlyArray<DistinctFinalityCut>) {
  it.effect(
