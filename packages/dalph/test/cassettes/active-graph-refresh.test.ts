...
import { activeGraphRefreshCassetteCatalog } from "../../test-support/active-graph-refresh-cassette.js"
import { runActiveGraphRefreshRestart } from "../../test-support/controlled-characterization.js"
...
for (const cut of ["Intent", "Observation"] as const) {
  it.effect(`recovers the same Run after G5 ${cut} while B C D remain held and F G remain unadmitted`, () =>
    Effect.gen(function* () {
      const result = yield* runActiveGraphRefreshRestart(cut)
...
it.effect("observes F and G without admitting either while B C and D retain every exact position", () =>
  Effect.gen(function* () {
    const result = yield* activeGraphRefreshCassetteCatalog.activeGraphRefresh.run
