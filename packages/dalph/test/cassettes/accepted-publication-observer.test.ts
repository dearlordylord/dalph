...
import {
  observeControlledActivationFailure,
  runAcceptedPublicationAcceptedPublicationObserver,
  validateControlledDs13Action
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const result = yield* runAcceptedPublicationAcceptedPublicationObserver
      ...
      for (const [name, action] of rejected) {
        ...
        const exit = yield* Effect.exit(
          observeControlledActivationFailure(validateControlledDs13Action(action, undefined, result.bRecords), failed)
        )
