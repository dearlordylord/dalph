...
import { retainedCCassetteCatalog, runRetainedCCassette } from "../../test-support/retained-c-cassette.js"
import { controlledDeliveryCharacterization as scenario } from "../../test-support/controlled-characterization-catalog.js"
import { runRetainedCCrashCheckpoint } from "../../test-support/controlled-characterization.js"
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const result = yield* runRetainedCCassette(retainedCCassetteCatalog.retainedCLifecycleReopen)
...
for (const checkpoint of [
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const result = yield* runRetainedCCrashCheckpoint(checkpoint)
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const result = yield* runRetainedCCrashCheckpoint("Pause")
      expect(
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const result = yield* runRetainedCCassette(retainedCCassetteCatalog.retainedCLostResumeResponse)
      expect(result.after.commands).toEqual([{ attemptId: scenario.attempts.C1, command: "Resume" }])
