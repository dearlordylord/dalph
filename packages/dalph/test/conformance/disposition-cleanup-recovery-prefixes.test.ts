...
const attempt = PlannedTaskAttempt.make({
  ...
  baseSha,
  branch: TaskBranchRef.make("refs/heads/task/cleanup-recovery-p1"),
  executor: TaskExecutorLocator.make("executor:cleanup-recovery"),
  runId,
  taskId: TaskId.make("cleanup-recovery-task"),
  taskRevision: TaskRevision.make("revision:1"),
  worktree: WorktreeLocator.make("/tmp/cleanup-recovery-p1")
})
const successor = PlannedTaskAttempt.make({
  ...attempt,
  attemptId: AttemptId.make("cleanup-recovery-p2"),
  branch: TaskBranchRef.make("refs/heads/task/cleanup-recovery-p2"),
  ...
  worktree: WorktreeLocator.make("/tmp/cleanup-recovery-p2")
})
const authorization = WorktreeCleanupAuthorization.make({
  ...
  operationId: OperationId.make("disposition-cleanup:worktree:cleanup-recovery-p1"),
...
const branchAuthorization = BranchCleanupAuthorization.make({
  ...
  observationAt: JournalPosition.make(30),
  observationOperationId: replacementWorktreeObservationOperationIdFor(attempt),
  operationId: OperationId.make("disposition-cleanup:branch:cleanup-recovery-p1"),
...
const candidateTarget = IntegrationTarget.make({
  ...
  repository: GitRepositoryLocator.make("repo:cleanup-recovery")
})
const candidatePredecessor = IntegratorSessionCorrelation.make({
  acceptedResult: candidateAcceptedResult,
  candidateResource: IntegratorCandidateResourceLocator.make("candidate:cleanup-recovery-p1"),
  expectedTargetHead: baseSha,
  integrationTarget: candidateTarget,
  plannedAttempt: attempt,
  queuedAt: JournalPosition.make(17),
  sessionId: IntegratorSessionId.make("session:cleanup-recovery-p1"),
...
const candidateAuthorization = IntegratorCandidateCleanupAuthorization.make({
  causalPredecessors: [OperationId.make("cleanup-recovery-full-rerun")],
  ...
  observationOperationId: OperationId.make("session:cleanup-recovery-p1:predecessor-lineage"),
  operationId: OperationId.make("disposition-cleanup:integrator-candidate:session:cleanup-recovery-p1"),
...
const acceptedJournalContextFor = Effect.fn("DispositionCleanupRecovery.acceptedJournalContextFor")(function* (
  journal: JournalStore["Service"]
) {
  const records = yield* journal.read(runId)
  const began = records[0]
  if (began?.event._tag !== "WorkflowRunBegan") return yield* Effect.die("cleanup recovery prefix lacks Run begin")
  ...
  return yield* Layer.build(journalLayer(runId, began.event.target, initial, journal))
})
...
const resumeCleanupAfter =
  ...
  ({ journal }) => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        const journalContext = yield* acceptedJournalContextFor(journal)
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          const final = responseLossCut ? yield* (yield* makeDispositionCleanupActivation(runId)).run : first
          const records = yield* journal.read(runId)
          const activeAuthorization = records.findLast(
