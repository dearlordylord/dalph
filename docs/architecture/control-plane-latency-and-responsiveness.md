...
| Execution start and stop observation | The execution substrate reports `ExecutorWorkExecuting`, `ExecutorWorkSafelySuspended`, or a terminal result for the exact `(RunId, AttemptId)`. | Passive observation has no retry budget; suspension retains its bounded command attempts; remote execution latency is metrics only. The selected Codex adapter separately bounds one local JSON-RPC acknowledgement wait. | Begin is once-only. Passive Observe does not consume a command budget. Suspension uses its existing positive command limit (`3` by default). One Codex JSON-RPC write-and-response wait is limited to `60 seconds`; a timed-out ambiguity-crossing request retains its durable intent and must be reconciled before any later command. Only an exact safe/terminal report releases task-work capacity. | The RPC limit does not bound an accepted task turn's execution time. A process timeout, missing response, missing session, or foreign report is not safe suspension, does not release a position, and does not authorize retransmission. |
...
Only the fourth interval has a V1 application-lifecycle deadline. The selected
Codex adapter also has the boundary-specific sixty-second JSON-RPC
acknowledgement deadline described above; it ends a local wait without deciding
the outside result. The first interval is a
