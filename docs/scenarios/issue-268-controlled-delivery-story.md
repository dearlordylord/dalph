...
## Fixed-base production-correction ledger

This ledger prevents the cassette from rediscovering or silently replacing the
production corrections selected by #314. `git cherry` against the fixed base
shows each source commit below as patch-equivalent to the listed fixed-base
commit even though the original SHA is not its ancestor.

| Accepted correction | Fixed-base equivalent | C1 executable verdict |
|---|---|---|
| `e84dd306f` full-capacity return | `ca5d849c6` | Re-run the owning #269 cassette-free test; do not redesign it in #268. |
| `7aff7a3ed`, `78c5aaa4c`, `ff97a5eb2` passive-observer retention and identity | `f22f30e65`, `c29b8963c`, `a79c82544` | Re-run the owning passive-retention tests. |
| `86137a865`, `709a830b7` current-first planning and coherent successors | `d5dc03b6f`, `b753d648a` | Re-run the owning planning-successor tests. |
| `e6160335c` same-position owner retention | `f9e269c82` | Re-run the owning same-position live-owner test. |
| `c1a3c981e` accepted-publication gating | `46236b1d6` | Re-run the owning accepted-publication test. |
| `2aae67f47` admission reconciliation | `390b08ad8` | Re-run the owning admission-reconciliation test. |

The active-refresh subject-retention patch `2732f6123` (patch-equivalent to
`96a1999fa`) is not present at the fixed base. C1 must first produce one direct,
cassette-free red/green verdict. Only the smallest proved patch may be replayed;
none of its cassette ancestry is an implementation input. Until that verdict,
its status is **candidate**, not missing accepted production behavior.
...
This ledger records patch presence, not correctness. C1 is complete only when
every row has a passing direct test on one clean descendant of the fixed base.
The representative fixed-base test inventory is:
...
| DS-02 | For A, then B, then C, Dalph reads the current task specification, records intent before adding the exact tracker claim, observes that claim, records the accepted plan and exact Base SHA, creates the one planned worktree, records one `Begin` intent at command/report ordinal 1, sends one `Begin`, observes `Executing` at ordinal 1, and publishes the exact attempt as holding one position. Gate R1 releases each task's boundary responses in that authored order. | A1, B1, and C1 execute concurrently and hold all three positions. Each has one claim, worktree, and `Begin`. |
...
| DS-04 | The edit notification is deliberately lost. The configured bounded timer fires. The sole refresh owner reads G1 once, then reads current specification, claim, worktree, and lineage facts for A, B, and C in graph order. A and C still match; B is F2. Dalph sends no executor command for A or C. It records one exact B1 `Suspend` intent and sends one `Suspend`. The immediate executor response remains `Executing`; no position is released. | The maintained delivery result projects B1 as suspending; A1, B1, and C1 still hold three positions. |
...
| DS-06 | The newly free position admits independent D. Dalph reads D's current specification, records and observes its exact claim, records D1's plan/Base SHA, creates D1's worktree, records one `Begin` intent, sends one `Begin`, observes `Executing`, and publishes D1 as holding the position. | A1, C1, and D1 execute and hold three positions; B1 remains retained; E remains unstarted. |
...
| DS-09 | Alice restarts Dalph on the same host. Dalph reads the Journal, reconstructs P2, A1/C1/D1 as held and B1 as retained, and reads the exact executor projections for A1, C1, and D1. All still report `Executing`. These passive reads consume no task-work position and send no `Begin` or `Resume`. Eligible E remains an exact unstarted proposal, but no fresh task is admitted because three positions remain held against capacity two. That non-empty capacity-blocked frontier makes the activation return exact `CoordinatorActivationReturned(RunMustRemainActive, RunnableTransition)` after the strict projections and without another graph read. | The maintained result again projects A1, C1, and D1 executing, B1 retained, and E unstarted, with capacity two. The sole owner settles the exact restart return before accepting the later close-C refresh. |
| DS-10 | Alice closes C in the tracker without reporting success. The tracker accepts G2 and sends one notification. The sole refresh owner reads G2 once and then reads the current specification, claim, worktree, and lineage facts for A, C, and D. A and D still match; C is closed. Dalph records one exact C1 `Suspend` intent, sends one `Suspend`, observes the immediate `Executing` response, and keeps C1's position held. | The maintained result projects C1 as suspending; A1, C1, and D1 still hold three positions. |
...
| DS-13 | Gate R11 lets A1's executor projection report terminal `Accepted`. Dalph records and publishes that exact result and releases A1's position. The already-selected exact B1 is considered before unstarted E, binds that position, records one `Resume` intent, sends one `Resume`, observes `Executing`, and publishes B1 as held. | B1 and D1 hold the two positions. A1 is accepted; C1 remains retained; E remains unstarted. No replacement B2 exists. Because E remains a capacity-blocked proposal, this beat does not fabricate a post-quiescence stabilization read. |
...
| R9 | C1 remains `Executing` through the `Suspend` response, then its passive lifecycle projection becomes `Safe`. |
...
not appended to DS-13: E is still an exact capacity-blocked proposal there, and
...
14. G2 and C's focused current facts precede C1's `Suspend` intent and call.
...
operations for D. DS-04 assigns O028--O031 to A, O032--O035 to B, and
O036--O039 to C. DS-10 assigns O064--O067 to A, O068--O071 to C, and
O072--O075 to D. DS-12 assigns O080 to the Continue record, O081--O082
to the graph read/publication pair, O083--O086 to B's four focused facts,
and O087 to the waiting projection.

| Beat | IDs | Occurrence groups | Count |
|---|---|---|---:|
| DS-01 | O001--O002 | G0 graph read; accepted graph publication | 2 |
| DS-02 | O003--O020 | A, B, C each: specification read; claim intent/call/observation; accepted plan/Base SHA record; worktree intent/call/observation; Begin intent/call/Executing observation; held-position publication | 18 |
| DS-03 | O021--O022 | Alice's F2 tracker edit; tracker acceptance | 2 |
| DS-04 | O023--O040 | refresh ownership; lost notification fact; bounded timer; G1 read; G1 publication; four focused current-fact reads each for A/B/C; B1 Suspend intent/call/Executing response | 18 |
| DS-05 | O041--O043 | B1 Safe projection; Safe Journal/publication; B1 position release/retention | 3 |
| DS-06 | O044--O049 | D specification read; claim intent/call/observation; accepted plan/Base SHA record; worktree intent/call/observation; Begin intent/call/Executing observation; held-position publication | 6 |
| DS-07 | O050--O051 | Alice's capacity request; durable P2 application | 2 |
| DS-08 | O052 | coordinator process loss | 1 |
| DS-09 | O053--O058 | Journal reconstruction; A1/C1/D1 passive projection reads; reconstructed state publication; exact `RunnableTransition` activation return | 6 |
| DS-10 | O059--O076 | refresh ownership; Alice's C close; notification; G2 read; G2 publication; four focused current-fact reads each for A/C/D; C1 Suspend intent/call/Executing response | 18 |
| DS-11 | O077--O079 | C1 Safe projection; Safe Journal/publication; C1 position release/retention | 3 |
| DS-12 | O080--O087 | Alice's exact Continue record; current graph read/publication; four focused B fact reads; waiting projection | 8 |
