...
The executor then reports B `ExecutorWorkTerminal` with an accepted result while C
remains `ExecutorWorkExecuting`. B's report releases one task-work position and
creates an integration obligation, but it neither proves B complete nor rereads
B's instructions to authorize work that has already finished executing. The graph
keeps B open and the Lab separately shows C as the remaining holder. C reports its
accepted `ExecutorWorkTerminal` result in a later production publication. After
both reports release capacity, Dalph rereads the current graph and claims, acquires
X's claim, reads X's exact current specification, and records X's first attempt
plan. It does not reread B or C's specifications as executor authorization.
