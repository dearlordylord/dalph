...
| Failures retain their exact tags/causes and D ambiguity follows reconcile-before-retry | `packages/orchestrator/src/coordination/delivery/run-delivery-runtime.test.ts` — `preserves post-G2 stall boundary failures without retry or fabricated quiescence`, `preserves exact post-G2 publication and executor failures with no E retry`, and `interrupts post-G2 D and releases its exact position without starting E`; `packages/orchestrator/src/coordination/run/run-stabilization.test.ts` — `preserves exact Journal and finality read failures before post-G2 admission without starting E`; plus the existing planned-attempt protocol conformance tests |
