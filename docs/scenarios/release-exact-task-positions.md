...
When the next result asks to integrate, preserve [accepted-result admission](queue-accepted-integration.md)
...
| Scenario | Acceptance test in `position-release.test.ts` |
| --- | --- |
...
| Maintained authored examples accept the immediate lineage call and publish real moments | `preserves maintained authored moments after fresh-claim integration progress` in `maintained-observations.test.ts`; Reducer Lab maintained-cassette smoke |
