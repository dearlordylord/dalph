...
first distinct non-action occurrence for the same pair. Later passive
...
- `drives one planned attempt through the generic executor boundary` proves
  Begin, executing, passive observation, and terminal projections using
...
send another Begin command. Production executors with independently surviving
state reconcile the exact command or passively observe the exact attempt as
specified by issue 264.
