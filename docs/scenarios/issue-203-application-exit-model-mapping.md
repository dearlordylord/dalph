...
| Atomic | `cutoffPreservesAnEarlierRegisteredOwnerTest`, `admittedAtomicBoundaryFinishesInsideTheOriginalDrainTest`, `fifthTickForceTerminatesAStuckAtomicBoundaryTest` | `lets an admitted atomic section return under Exit and starts no successor phase`, the three-family cassette/classification tests, `forcefully terminates at five seconds while an atomic integration section remains active`; production-backed MBT | #207 implemented production return/stuck cut points; #208 separately owns cleanup boundaries |
