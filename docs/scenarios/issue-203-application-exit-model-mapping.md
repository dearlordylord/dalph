...
| Atomic | `cutoffPreservesAnEarlierRegisteredOwnerTest`, `admittedAtomicBoundaryFinishesInsideTheOriginalDrainTest`, `fifthTickForceTerminatesAStuckAtomicBoundaryTest` | `lets an admitted atomic section return under Exit and starts no successor phase`; `replays the authored candidate Exit cassette through production admission without a successor`; `replays the authored verification Exit cassette through production admission without a successor`; `replays the authored promotion Exit cassette through production admission without a successor`; `defects when an atomic owner reaches an interruptible route while Serving`; `defects when an interruptible owner reaches an atomic route while Serving`; `forcefully terminates at five seconds while an atomic integration section remains active`; production-backed MBT | #207 implemented production return/stuck cut points; #208 separately owns cleanup boundaries |
...
| Failure | `conclusiveFailureWaitsForIndependentQuickDrainWorkTest`, `usefulQuickWorkBlocksConclusiveFailureTest`; early-failure and zero-status negative controls | failure guard and forced process-end unit tests; production-backed MBT | #209 cross-family concurrent runtime drain test; #208 proves exact cleanup preservation without inventing cleanup work |
