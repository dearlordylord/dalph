...
target-promotion attempts share the same operation family but retain distinct
...
3. Alice moves Back or Forward to another production cursor. If the selected
   ...
   tracker task id.
4. Alice enables “Fold repeated observed attempts.” Reducer Lab replaces each
   consecutive presentation group with one summary row that states its count
   and first/last journal positions. Expanding the group reveals every exact
   original row and payload. Disabling folding restores the same identities in
   the same order. Initiated actions and non-action observations are never
   folded together, and actor attribution remains the occurrence's truthful
   classification.
...
focus a task or occurrence, fold noisy repeated attempts reversibly, and read
...
initiated action with a non-action observation, preserve a selection that is
...
- `folds repeated observed attempts without losing occurrence identity` proves
  reversible folding, first/last position summaries, exact expansion order,
  and classification/actor separation.
