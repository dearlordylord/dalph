...
| A result returns inside the original drain, records the family result, releases the owner, and starts no successor | `records the constructed candidate through production admission before Exit stops the successor`; `records verified evidence through production admission before Exit stops the successor`; `records an applied promotion through production admission before Exit stops the successor`; `lets an admitted atomic section return under Exit and starts no successor phase` |
...
| Actor-readable and recorded chronology preserves candidate, verification/evidence, and promotion facts without an Exit workflow event | The three production-admission tests above run the real integration adapter and assert `IntegrationCandidateConstructed`, `TargetVerificationEvidenceSealed`, or `TargetPromotionObservedSuccess` in the journal while the post-adapter successor remains at zero; existing maintained cassettes separately prove the detailed family histories in `runs maintained conflict, unreadable-Git, correction, exhaustion, and contradiction stories`, `runs only the selected public wrapper and seals passing evidence for exact M`, `promotes verified M by exact compare-and-set and records exact ancestry`, and `reconciles a lost promotion response and never sends a fourth request` |
