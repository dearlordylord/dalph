...
| A result returns inside the original drain, records the family result, releases the owner, and starts no successor | `replays the authored candidate Exit cassette through production admission without a successor`; `replays the authored verification Exit cassette through production admission without a successor`; `replays the authored promotion Exit cassette through production admission without a successor`; `lets an admitted atomic section return under Exit and starts no successor phase` |
...
| Actor-readable and recorded chronology preserves candidate, verification/evidence, and promotion facts without an Exit workflow event | Each production-admission test above compares its authored chronology with entries recorded from the real boundary entry, cutoff request, exact durable journal result (`IntegrationCandidateConstructed`, `TargetVerificationEvidenceSealed`, or `TargetPromotionObservedSuccess`), and released owner with a zero successor count; existing maintained cassettes separately prove the detailed family histories in `runs maintained conflict, unreadable-Git, correction, exhaustion, and contradiction stories`, `runs only the selected public wrapper and seals passing evidence for exact M`, `promotes verified M by exact compare-and-set and records exact ancestry`, and `reconciles a lost promotion response and never sends a fourth request` |
| A route/boundary classification bug is an invariant defect rather than an Exit interruption | `defects when an atomic owner reaches an interruptible route while Serving`; `defects when an interruptible owner reaches an atomic route while Serving` |
| Production decision conformance reaches atomic return and stuck timeout while workflow Exit history stays empty | The three production-admission tests above prove the concrete family wiring; `replays application Exit decisions through the production lifecycle kernel` plus canonical `admittedAtomicBoundaryFinishesInsideTheOriginalDrainTest`, `fifthTickForceTerminatesAStuckAtomicBoundaryTest`, and `everyExitOutcomeLeavesWorkflowExitRecordCountZeroTest` prove the shared decision projection |
