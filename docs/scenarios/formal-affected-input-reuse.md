...
| Change a generated pnpm launcher's target or add a runtime command | `formal-input-policy.test.mjs`: `builds one applicability identity for equivalent inputs and checkout tools in relocated worktrees` asserts both changes produce a different applicability digest |
| Try to reuse from another clone, after the host boot changes, after the origin worktree disappears, or while original process custody is unsafe | `formal-success-evidence.test.mjs`: `reuses stopped evidence across same-custody worktrees and rejects unsafe original custody` asserts each case is a miss |
