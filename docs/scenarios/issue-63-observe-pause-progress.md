...
Status: accepted chronological scenarios for issue #63. The maintainer accepted
the user-oriented Pause observation contract on 2026-08-10: the applied Pause
remains durable across restart, while this passive progress view is always
derived and never persisted as a second pause authority.
...
The accepted observation is transport-independent. After one exact Run or task
...
The maintainer accepted these public output choices:
...
The implementation handoff must map every named test above to a passing
...
Adding this accepted file and its scenario-index entry changes no Dalph runtime
