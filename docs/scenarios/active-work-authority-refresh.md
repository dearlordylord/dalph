...
timer](reactivate-incomplete-runs.md#a-lost-tracker-notification-is-recovered-by-the-bounded-timer):
...
check](reactivate-incomplete-runs.md#several-hints-produce-one-activation-and-one-optional-trailing-check)
...
B](refresh-complete-task-pipelines.md#a-later-recorded-tracker-observation-releases-task-b)
remains the ordinary complete refresh-and-traversal owner, and
[A fresh read finds unchanged
facts](journal-first-tracker-observations.md#a-fresh-read-finds-unchanged-facts)
...
B](stabilize-each-run.md#g2-is-requested-only-after-g1-is-quiescent-and-reveals-b)
...
running](reconcile-changed-task-facts.md#alice-changes-as-instructions-while-its-planned-attempt-is-running),
[Another tracker client replaces A's claim while A is
running](reconcile-task-claims.md#another-tracker-client-replaces-as-claim-while-a-is-running),
[Scenario 12B: the integration target is rewritten outside the planned
lineage](reconcile-git-facts.md#scenario-12b-the-integration-target-is-rewritten-outside-the-planned-lineage),
[Scenario 14A: Git no longer registers the planned
worktree](reconcile-git-facts.md#scenario-14a-git-no-longer-registers-the-planned-worktree),
...
one live request](run-reactive-delivery-actions.md#one-persistent-github-claim-proposal-starts-one-live-request).
[A newer causal route does not repeat the same recovered boundary
read](run-reactive-delivery-actions.md#a-newer-causal-route-does-not-repeat-the-same-recovered-boundary-read)
