...
## Maintained scenario-to-test and model map

The following map is the handoff ledger for this issue. The `P0`--`P6`
labels are applicability labels for the executable `gitReconciliation` model;
they are not production phases, persisted workflow states, or tracker
identities. `P0` is the pre-promotion blocker, `P1` is its clear-and-reread
path, `P2` is stale-session supersession, `P3` is the post-promotion blocker,
`P4` is promoted ancestry confirmation, `P5` is incomplete authority, and
`P6` is the completion race.

| Accepted seam | Production test | Authored/recorded cassette | Model and executable adapter | Deliberately unperformed effect / owner |
| --- | --- | --- | --- | --- |
| `preserves the candidate and releases integration when a blocker appears before promotion`; `keeps the prerequisite and independent task eligible during the local wait`; `reopens the pre-promotion blocker wait without derived resource ownership` | `preserves the candidate and releases integration when a blocker appears before promotion` | `prePromotionBlockerAuthoredCassette` and `blockersAroundPromotionAuthoredCassette`, projected by `projectRecordedCassette` | `prePromotionBlockerPreservesCandidateAndReleasesTargetTest` (`P0`), `prePromotionBlockerRecoveryPreservesCandidateAndEmptyTargetTest` (`P0`/`P5`) in `gitReconciliation.mbt.test.ts` | B's own work and C's work are ordinary graph-frontier work; no A-owned claim or wait is created (D9/D18/D19/D29/D42/D43) |
| `freshly proves Git authority after a pre-promotion blocker clears`; `clears dependency wait when a complete read proves prerequisite edge was removed`; `retains the complete observations needed to derive a future removed-prerequisite warning`; `supersedes the stale-head session before starting one successor from the new head`; `clears only the dependency constraint and does not promote directly` | `supersedes a stale pre-promotion session before starting one successor` and `does not let an unrelated task in the same run perturb the successor ordinal` | The same authored cassette records the blocker graph; the recorded projection retains both complete graph observations and the `IntegrationCandidateSessionSuperseded` history when the stale-head fixture is replayed | `prePromotionClearRereadsAndSupersedesBeforeOneSuccessorTest` (`P1`/`P2`) plus `integration-history.test.ts`'s `rejects two different successors for one prior candidate session` | Candidate construction, verification, and promotion remain owned by the existing bounded protocols (#57/#59/#60); this issue does not mutate M or promote merely because B clears |
| `preserves promotion proof and waits before tracker completion on a new blocker`; `never rolls Git back after promotion`; `keeps prerequisite and independent work eligible after promotion` | `preserves promotion proof and waits before tracker completion on a new blocker` | `blockersAroundPromotionAuthoredCassette` records the successful promotion followed by a complete blocker graph and its recorded projection preserves the promotion proof | `postPromotionBlockerPreservesProofAndWaitsTest` (`P3`) | No rollback, rebuild, re-verification, or second compare-and-set is authorized (D10/D16/D27/D28) |
| `proves promoted candidate ancestry after the blocker clears and completes without reintegration`; `waits without tracker completion when promoted ancestry unreadable or incompatible` | `records completion finality after valid candidate verification and promotion history` plus the completion-task authorization tests | `deliveryFinalitySpineAuthoredCassette` supplies the current-head Git read; its recorded projection retains `CompletionTaskCandidateAncestryReadIntended`/`Observed` | `clearedPromotionRequiresFreshAncestryAndNeverReintegratesTest` (`P4`) and the completion-task authorization tests | Completion Q and claim cleanup remain the #61/#141 finality protocols; this issue never manufactures a replacement integration candidate |
| `preserves the current integration state when blocker facts are incomplete`; `releases target ownership during an incomplete tracker wait without same-target reordering`; `does not treat omitted prerequisite facts as blocker clearance` | `unreadable task Unpause remains unapplied when the fresh read is incomplete` plus the focused frontier regression | `unreadableTaskUnpauseRejectedAuthoredCassette` covers the authored incomplete tracker boundary; its recorded form retains no inferred clearance | `incompleteTrackerFactsReleaseTargetWithoutTransitionTest` (`P5`) | The tracker-read protocol owns retry/reconciliation of an incomplete result; no blocker or clearance is inferred (D23/D24) |
| `preserves accepted tracker completion when a prerequisite concurrently reopens`; `warns about completed A with newly unfinished prerequisite without automatic repair`; `clears the derived warning when fresh tracker facts remove the inconsistency` | `preserves accepted tracker completion when a prerequisite concurrently reopens` | `prerequisiteReopensDuringCompletionAuthoredCassette` and its recorded projection retain acknowledgement, focused success, and later graph facts | `completionRacePreservesAcceptedCompletionAndDerivedWarningTest` (`P6`) | The tracker owns accepted lifecycle facts and any visible warning projection; Dalph does not reopen A, claim B, reverse Git, or persist warning state (D42/D44) |

The first four rows use complete tracker graph observations to prove a
transition. The `P5` row intentionally uses a typed incomplete observation and
therefore proves only preservation and release. The `P6` row treats tracker
acknowledgement as an acknowledgement, then requires a focused successful
completion observation before exposing the later inconsistency.
