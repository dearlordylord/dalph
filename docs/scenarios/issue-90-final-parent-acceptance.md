...
1. `scripts/capability-registration.ts` owns the 17 production-capability
...
- #79 mechanically registers the capability families and makes the gate part
  of `check:all`; #285 split GitHub completion-claim and task-completion
  authority into distinct registrations, and #307's explicit Codex-owned
  activity census extends the denominator to 17.
...
- `runs every registered controlled, production, and qualification implementation
  ...
  the closed 17-family composition denominator.
