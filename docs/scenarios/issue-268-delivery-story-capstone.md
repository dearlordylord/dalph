...
| DS-01 through DS-13 retain exact Run, attempts, Base SHA, claims, worktrees, capacity, held/retained states, and accepted outcomes through all three cuts | Blocking assertions-lane integration dependency: the settlement lane compiles the capstone bodies conditionally but does not claim the literal checkpoint table; the composed branch must supply and execute the DS-01 through DS-13 manifest proof |
