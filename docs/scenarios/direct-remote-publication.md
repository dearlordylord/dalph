...
**Status: accepted by the maintainer on 2026-09-19; the maintained capstone chronology repair, controlled S1/S7 checks, and a prior frozen local full gate are recorded. The final candidate still requires an exact-head full gate and a successful fresh supervised disposable S1 dogfood.** Issue #390's retained-run closure is complete; the supervised S1 must be a fresh disposable run and must not repair the retained failed candidate.
