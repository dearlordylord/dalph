...
Status: proposed on 2026-08-08. The first four maintainer decisions were
...
## Proposed vocabulary, awaiting acceptance

This proposal uses `PlannedAttemptReplaced` for the proposed workflow event
...
recorded applied Restart direction, unbroken exact safe-suspension evidence,
...
This proposal would also expand the canonical attempt-choice request identity
from Continue and Stop to Continue, Restart, and Stop for one exact
...
fingerprint pair, or choice is contradictory. Neither proposed definition is
canonical in `docs/CONTEXT.md` until the full scenario is accepted.
...
become “the Restart direction and one proposed `PlannedAttemptReplaced` event
are durable.” That event makes exact P1 no longer unsettled while it records
...
## Next maintainer decision: a late Accepted report for P1

After the Restart direction carried by D1 is committed in the Journal but
...
report for P1 may arrive. The accepted safe-suspension rule means that report
cannot authorize P2. The proposal below currently recommends preserving the
commit as P1 evidence, creating no integration responsibility, and returning a
typed no-replacement result for D1. This requires an explicit amendment to
issue #56's current rule that an accepted terminal reconstructed from Journal
history receives its missing integration responsibility exactly once.

The competing choice is to let exact `Accepted` enter issue #56's ordinary
path: Dalph would create P1's integration responsibility, the integration
cutoff would prevent P2, and the already-applied Restart direction carried by
the request identity D1 would cause Dalph to return the same typed
no-replacement result correlated with D1. Under either choice, Dalph ends P1's
planned-attempt executor-work
responsibility, preserves the terminal report, and does not append
`PlannedAttemptReplaced`; the maintainer must choose whether the accepted
commit starts integration or remains preserved evidence before the scenario
can be accepted.
...
3. Dalph checks that P1 is still before integration, that the Journal contains
   ...
   Alice's applied Restart direction before selecting any replacement work.
...
6. Because no later executor command broke the exact safe-suspension proof,
   ...
   direction carried by D1, the recorded current exact `Planned worktree ready`
   ...
   P2 once and constructs one proposed `PlannedAttemptReplaced` event. It
...
reads, allocates a fresh proposed successor identity, and attempts a new
...
Alice sees the Restart direction carried by D1 applied, P1 durably superseded,
...
The current invariant list does not yet state four other forbidden results in
this chronology. Before accepting this scenario, the delivery specification
must state that only the exact applied Restart direction, unbroken safe-
suspension evidence, the recorded current exact W1 `Planned worktree ready`
...
P1 no longer unsettled and records P2; `Completed`, `Failed`, and `Accepted` do
not authorize that event; Restart retains K1 and sends no claim mutation; and
W2 receives no content from W1 while W1 remains preserved for a separately
authorized disposition. Under the currently
recommended unresolved choice, the specification must also say that a terminal
`Accepted` report arriving after the Restart direction carried by D1 was
committed and before integration starts remains preserved evidence and starts
no integration responsibility. D49 must
cover exact Restart redelivery and first-journaled arbitration with Continue
and Stop. Until those invariant amendments are accepted, this proposal remains
incomplete and cannot authorize implementation.
...
Dalph has recorded the applied Restart direction carried by D1, but no
...
merely because the Restart direction carried by D1 was applied, infer readable
...
Dalph has durably recorded the applied Restart direction carried by D1, but a
...
planned-attempt executor protocol. Its direct result or later
...
Only an exact `SafelySuspended` report restores the proof used by the first
scenario and permits the remaining current tracker and Git reads that may lead
to `PlannedAttemptReplaced`. Exact terminal `Completed` and `Failed` reports
also prove that no P1 writer remains, but the settled rule does not treat that
as replacement authority. Each exact terminal report ends only P1's
planned-attempt executor-work responsibility and remains preserved under its
distinct result. `Completed` and `Failed` neither prove A completed in the
tracker, supply an accepted Git result, nor permit P2. A late terminal
`Accepted` report preserves its exact commit as P1 evidence but likewise cannot
authorize P2. Whether that accepted result creates issue #56's ordinary
integration responsibility remains the explicit unresolved choice above. Under
the proposal's current recommendation, Dalph does not pair the commit with a
target or create an integration responsibility; accepting that choice requires
amending issue #56's scenario and conformance seams. `Completed`, `Failed`, and
`Accepted` each return a typed no-replacement result for D1. Dalph never
rewrites one terminal variant as another or appends `PlannedAttemptReplaced`
from terminal evidence.

After a crash, the next invocation establishes the Run, reconstructs the
unresolved suspension command, and ordinary activation checks the executor
before another state-changing request. In the controlled fake composition, the
fake dies with Dalph and is recreated for the same R/P1 protocol; Dalph does
not claim an old response survived. Process loss, timeout, missing session
data, and a report for another attempt prove no writer stopped.
...
its own name with a typed no-replacement result. Dalph must not release K1,
discard W1, record planned task attempt P2, or return a successful replacement
result for D1 while a running, terminal, contradictory, or unreadable result
lacks the exact unbroken `SafelySuspended` proof. It must not call A completed from
`Completed` or `Failed`, and under the currently recommended unresolved choice
it must not integrate the late `Accepted` commit.
...
- `does not authorize PlannedAttemptReplaced from Completed Failed or Accepted`
- `preserves a late Accepted commit without creating integration after the
  Restart direction carried by D1 was committed`
- `does not append issue 56's integration responsibility when the Restart
  direction carried by D1 was committed before a late Accepted terminal`
...
redelivery of D1 whose Restart direction was committed before the cutoff still
...
W1 and K1, and allow that event only after exact unbroken safe suspension, a
recorded current exact W1 `Planned worktree ready` observation containing H1
...
integration after the Restart direction carried by D1 was committed; no
terminal variant authorizes `PlannedAttemptReplaced`; and integration start
removes the Restart capability.
...
responsibility when the Restart direction carried by D1 was committed before a
...
The proposed vocabulary section above, rather than `docs/CONTEXT.md`, names the
three-choice request identity expansion and `PlannedAttemptReplaced` while the
scenario remains unaccepted.
...
the recorded applied Restart direction carried by D1, the later claimed-task
