...
- [Issue #264: Begin once, then observe the same executing work](autonomous-executor-work.md#begin-once-then-observe-the-same-executing-work)
  and [Resume only the same safely suspended attempt](autonomous-executor-work.md#resume-only-the-same-safely-suspended-attempt-selected-by-current-facts)
...
- [Issue #265: a later process reattaches to the exact attempt](passive-executor-observation-through-restart.md#a-later-dalph-process-reattaches-to-the-exact-codex-attempt)
  owns same-host restart observation without another work command. This
  scenario preserves it for A1, C1, and D1 together.
- [Issue #266: Alice changes B while A1, B1, and C1 execute](active-work-authority-refresh.md#alice-changes-b-while-a1-b1-and-c1-execute-autonomously)
...
- [Issue #267: maintained production chronology](exact-causal-active-work-cassette.md#maintained-production-chronology)
...
- [Issue #269: read-only restart obligations do not block D](independent-work-retained-priority.md#read-only-restart-obligations-do-not-block-independent-d)
  and [exact B1 selected for Continue precedes fresh work](independent-work-retained-priority.md#exact-b1-selected-for-continue-precedes-d-and-replacement-b2)
...
- [Issue #315: preserve bounded fresh admission through exact handoff](preserve-bounded-fresh-admission.md)
...
parameterized negative test `rejects each required controlled edge reversal`,
...
| DS-12 | Alice's exact Continue and current B facts produce a B1 Resume proposal, but B1 waits without authorization or Resume while A1/D1 fill capacity two. |
...
- after the second acceptance cut, `consumes exactly the accepted controlled
  occurrence inventory`;
- `repeats the complete delivery checkpoint table twenty times with
  identical observed order`.

The delivery-repeatability qualification (historically checkpoint C4 in the
owning issue) starts twenty fresh Vitest processes sequentially.
...
test-specific
`controlledDeliveryCassetteCatalog.controlledDs01ThroughDs13` catalog
seam. They
...
| C4 — delivery repeatability | Twenty consecutive uncached full runs consume the identical accepted observed order and pass all 13 rows | 0.5 working day | One readiness-control reconsideration is allowed; another divergence freezes the lane. |
