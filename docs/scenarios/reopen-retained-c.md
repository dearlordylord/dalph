...
protocol](autonomous-executor-work.md#resume-only-the-same-safely-suspended-attempt-selected-by-current-facts)
and [tracker lifecycle reconciliation](reconcile-changed-task-facts.md#the-tracker-closes-a-without-success-and-later-reopens-it).
...
| G4 clears lifecycle only; B/D retain capacity; original C resumes after revision three | `lifecycle-resume.test.ts`: `reopens C and resumes its original attempt only after accepted capacity three` |
| Capacity two rejects C reads; capacity three reserves one position before E | `capTwoRejectsCReservationTest`, `capTwoRejectsFocusedCReadTest`, `capThreeRejectsFreshEBypassTest`, `cReservesThirdPositionBeforeFreshETest`; controller tests `reserves only the third position for Safe revalidation before fresh E and releases a rejected pre-intent lease` and `admits Safe continuation rereads in responsibility chronology` |
...
| Same reserved position spans reads, authorization, and exact Resume; stale retry basis is rejected | `reservedCRejectsEarlyResumeTest`, `reservedCRejectsMissingWitnessAuthorizationTest`, `reservedCRejectsDoubleOccupancyTest`; controller tests `keeps one exact safe continuation reservation through reads and Resume intent handoff`, `rejects stale or swapped reconciled Resume identity before reserving`, `reconciles a reserved Safe continuation only with the same accepted attempt`, and `rejects stale, foreign, and wrong-kind accepted Resume handoffs without minting position authority` |
...
| Executor applies Resume but its response is lost | `lifecycle-resume.test.ts`: `reconciles C's lost Resume response after restart without another Begin or Resume` |
| Crash before the initial Resume call; Safe reconciliation permits only same-ordinal redelivery | `lifecycle-resume.test.ts`: `recovers the real ResumeIntent crash before resuming exactly retained C1`; pure `resume-redelivery-authorization.test.ts` rejects consumed/swapped/foreign or superseded proofs and rejects redelivery when the immutable Run target or accepted Resume intent is absent; controller exact receipt tests retain one position across append publication and reject settled, stale, foreign, or wrong-kind handoffs |
| Crash after redelivery intent, after Held handoff, or after retry effect before response | `lifecycle-resume.test.ts`: `recovers RedeliveryIntent with a new projection and no new semantic Resume`, `recovers RedeliveryHeld with a new projection and no new semantic Resume`, and `recovers RedeliveryResponseLost with a new projection and no new semantic Resume`; each validates B/C/D reconstructed positions and unchanged resources |
...
| Generic activation recovery cannot bypass the crash that invalidates an uncalled Resume's permission | `activationRecoveryCannotBypassUncalledResumeCrashTest` and `activationRecoveryCannotBypassResumeRedeliveryCrashTest` reject the alternate recovery edge; `activationRecoveryRetainingResumePermissionIsDetectedTest` breaks the independently gated `activationRecoveryCannotRetainResumeCallPermission` law; the existing redelivery crash test follows `crashResumeDelivery` and obtains a new projection, witnesses, and intent |
