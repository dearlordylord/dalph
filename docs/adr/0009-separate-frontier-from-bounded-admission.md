...
After executor-responsibility handoff, task A keeps its task-work position until
the executor returns a terminal result for that complete planned attempt or
proves the complete attempt safely suspended after Dalph asked it to stop for
later resume.
...
safely suspended, or a conclusively unaccepted first intent releases its exact
entry reservation—the Dalph
...
The controller returns an atomic admission decision and exact entry
reservations; it does not expose a dormant `awaitAdmission` operation.
...
Entry reservation, ownership registration, and scoped-runner start form one
interruption-masked handoff. An unsuccessful handoff makes its exact newly
reserved entry available before returning or dying.
...
recorded request and retains its admission commitment. Exhausted shared-boundary
