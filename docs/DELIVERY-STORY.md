...
- `emits the exact DS01 through DS13 delivery checkpoint table` derives each
  ...
  Journal positions.
- `captures B's exact F1 F2 choices from each production runtime observation
  through Continue` checks the actual runtime-observation evaluation's current
  ticket-delivery constraint at every
...
- `rechecks after a lost notification when TestClock fires, with no Run read
  ...
  notification arrives. `accepted B F2 refresh suspends only B1 while A1 and C1
  continue executing` separately proves that a timer-sourced production refresh
  observes B's changed instructions, suspends B, and leaves A and C executing.
  The two tests provide compositional lost-notification/timer and B/F2 evidence;
  neither is represented as one combined execution, and the cassette does not
  relabel authored hints as a substitute.
...
- `observes reduced capacity revision two before the authored restart cut` and
  `returns RunnableTransition after strict restart projections before the
  queued G1 refresh` check revision 1→2 with no revision 3, then the
  graph/A/C/D-projection/RunnableTransition restart sequence with no durable
  Begin/Resume command or active-refresh authority chain at that cut. The first
  hints admit the separate queued G1 refresh and its exact independent A/C/D
  specification-to-lineage chains; the same activation then performs DS-10's
  post-quiescence G2 refresh. After the A/D chains and C2 Safe, its
  UnsettledResponsibility return settles before Continue B. The exact eighteen-member,
  fifteen-edge structure proves B/E exclusion and cross-lane incomparability
  without re-enumerating all 17,153,136 legal orders already governed by the
  generic matcher tests.
...
DS-09|DemonstratedByMaintainedSlice|authored:autonomousExecutorDeliveryCapstone|packages/dalph/test/cassettes/delivery-story-capstone.execution.test.ts#it.effect#emits the exact DS01 through DS13 delivery checkpoint table,packages/dalph/test/cassettes/delivery-story-capstone.execution.test.ts#it.effect#captures B's exact F1 F2 choices from each production runtime observation through Continue,packages/dalph/test/cassettes/delivery-story-capstone.execution.test.ts#it.effect#retains exact Run attempt claim and resource identities across DS01 through DS13,packages/dalph/test/cassettes/delivery-story-capstone.execution.test.ts#it.effect#represents queued G1 as three independent A C D authority lanes before G2,packages/dalph/test/cassettes/delivery-story-capstone.execution.test.ts#it.effect#returns RunnableTransition after strict restart projections before the queued G1 refresh,packages/orchestrator/src/coordination/delivery/run-delivery-runtime.test.ts#it.effect#publishes current-first exact live-owner observations until standalone runtime quiescence,packages/dalph/test/cassettes/scenario.test.ts#it.effect#reuses one delivery relation evaluation for the authored publication frame
