...
| Q creates its first disposable resource. | The live controller writes a redacted retained-resource checkpoint immediately after exact issue creation, replaces it after local fixture creation, and removes it only after successful final publication. The always-upload step publishes the last completed checkpoint after outer cancellation. | `pnpm exec vitest run packages/dalph/test-support/production-live-qualification-runtime.test.ts -t 'persists the exact remote fixture|unfinished recoverable Run'`. Target: zero live cancellations with created Q resources and no uploaded disposition artifact. |
| A qualification wrapper launches a package-manager shim but ownership is recorded against the wrapper. | Before spawning the controller, require the derived locked JavaScript entry to be readable and nonempty. Generate a Bash wrapper that rereads it, invokes it through the exact Node executable, and preserves the wrapper locator as `argv[0]`; keep the existing exact matcher unchanged. | `scripts/run-production-live-qualification.test.mjs` rejects a missing derived entry before child launch. `packages/dalph/test-support/production-live-qualification-runtime.test.ts` proves the generated command retains the expected wrapper identity, the exact matcher accepts that post-exec command, and rejects the former `node …/codex.js app-server` identity. Target: zero owned app-server closes rejected because a generated shim changed only `argv[0]`. |
