...
| Dalph executor | complete work for one planned attempt and its normalized executing, safely suspended, or terminal report | exact planned-attempt correlation and journaled responsibility/report facts |
...
application-lifecycle facts outside every Run workflow journal. In the ordinary
...
lock. Registered process-local quick-drain close operations run before that
result: `Succeeded` proves those operations closed their resources; `Failed`
proves useful operations settled with diagnostics, but not necessarily that
every resource closed; and `TimedOut` may leave unresolved resources for later
host scope finalization. Returning from the host callback then closes
remaining scoped resources and releases that lock, without a process-end
capability or event. A conclusive failure and a five-second unresolved drain
force-terminate only through the ordinary process-exit shell; host-scoped
failure/timeout results remain non-graceful and are followed by ordinary scope
finalization. Restart restores no Exit mode or timer and uses ordinary Run
