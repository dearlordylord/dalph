...
| **Accepted locally — publication pending** | The implementation evidence is committed, reviewed, verified, and explicitly accepted by the issue owner, but has not been pushed and remotely reread. Record the local commits, acceptance evidence, reviews, and gate without claiming repository-proven closure. |
...
| Add transitional source comments | **Resolved — repository proven** | The #132/#133 comments in `runnable-frontier.ts`, `reconstructed-run-state.ts`, and `task-admission-controller.ts` were reviewed, committed in `f6118dbe3186d7f2e106355f35f9ca1e7cd3fd69`, pushed, and remotely reread. They change no runtime behavior. |
...
| Persist the owner's responsibility-order decision | **Resolved — repository proven** | The owner accepted Option B on 2026-07-26. The live [#132](https://github.com/dearlordylord/dalph/issues/132) body was updated and reread: whenever a controller-snapshot change can permit admission—including confirmed provider non-consumption or reservation release/cancellation—the coordinator reads current reconstructed run state plus the snapshot and derives again; no dormant waiter supplies a second order. The canonical specification and ADR 0009 were reviewed, committed in `f6118dbe3186d7f2e106355f35f9ca1e7cd3fd69`, pushed, and remotely reread. Implementation remains open under #132. |
| Design issue #132 activation ownership | **Resolved — repository and tracker proven** | Human grilling in [Define exact activation ownership and admission handoff](https://github.com/dearlordylord/dalph/issues/151) accepted the [exact activation ownership and admission handoff](issue-132-activation-ownership-decision.md) and [implementation handoff](issue-131-handoffs/issue-132-implementation.md). Commits `1f32664c6`, `7f19adab9`, `522cb27c5`, and `6465ff9f8` record the reviewed design: one activation coordinator and capacity-N overlapping owned-operation runners; internal reserved-position accounting; independently modeled coordinator and provider-worker failure; non-preemptive restart capacity; classified subject-local duplicate-ownership defects; and fully MBT-testable M2 profile decomposition if state explosion occurs. Independent domain/spec and architecture/standards reviews reported no findings after correction, and `pnpm check:all` passed. The live implementation ticket records the accepted design, the Wayfinder decision is closed with its resolution, and the map links that resolution. |
...
| “Activation owner reconstructs inputs” hid the actor/boundary and could require full restart reconstruction on every release. | Accepted. Canonical prose now names any controller-snapshot change that can permit admission, including provider non-consumption and reservation release/cancellation; the coordinator then reads current reconstructed run state plus the snapshot, with workflow-selected external rereads only when knowledge is insufficient. |
...
run state plus the controller snapshot and derives the frontier again.
...
reads current reconstructed run state plus the controller snapshot and
...
- **Accepted behavior:** Whenever a controller-snapshot change can permit
  ...
  release/cancellation—the coordinator reads current reconstructed run
...
## Additional evidence notes and open gaps

These were discovered during research and are not substitutes for F1–F26 or
U1–U8:

1. **Capacity-one model evidence (owner accepted; repository publication
   pending):** `CAPACITY` is now supplied by the checking profile.
...
| SF24 — Show both U3 behaviors before asking | **Resolved — repository proven** | The owner accepted Option B on 2026-07-26 after reviewing both traces. The live #132 issue was updated and reread: whenever a controller-snapshot change can permit admission, the coordinator reads current reconstructed run state plus the snapshot and derives again. The canonical specification, ADR 0009, ledger, and H2 changes were reviewed, committed, pushed, and remotely reread in `f6118dbe3186d7f2e106355f35f9ca1e7cd3fd69`. Implementation remains open under #132. |
...
H3 is handoff-ready from H2's design. Capacity-one evidence acceptance is no
longer its blocker. Its required return includes the production activation API,
model actions for derive/reserve/ownership/interruption/result/release/
reconstruction, counterexamples for duplicate ownership and leaked
reservation, production-facing MBT, generated command sequences, memory/SQLite
