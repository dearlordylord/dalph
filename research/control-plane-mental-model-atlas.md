...
| Independent production executor restoration, including live-process adoption or proof of safe stop | Required direction, but detailed executor internals are explicitly post-milestone design | **No.** Production composition injects an opaque executor; Dalph does not select or inspect an implementation | **No production proof.** Current tests and Quint model prove only the opaque outer executor contract | [durability qualification](../docs/ARCHITECTURE.md#durability-and-reconstruction), [controlled test support](../packages/orchestrator/test/controlled-planned-attempt-executor.ts) |
