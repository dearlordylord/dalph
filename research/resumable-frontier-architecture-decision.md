...
| Existing focused protocol, Schema, property, SQLite, and interpreter-equivalence tests | Retain where their public seam survives | Keep tests that prove an exact retained boundary contract. Replace tests that assert `ManagedRunRecoveryStageEntry`, one-shot traversal, or global startup blockage. Every behavior slice also updates the owning Quint model, public adapter, semantic trace, and applicable in-memory and SQLite P0–P6 prefixes. |
