...
| Safe suspension of one exact attempt | Yes | Yes only at the generic boundary and controlled fake | Yes for that milestone contract | The fake reports `SafelySuspended` only for the same `(RunId, AttemptId)`, and generic orchestration retains capacity until that report ([fake executor](../packages/executor/src/controlled-fake.ts), [accepted scenario](../docs/scenarios/planned-attempt-executor-boundary.md)). No production provider/process has yet proved that its agent context, logs, worktree state, and activity are safely resumable. |
