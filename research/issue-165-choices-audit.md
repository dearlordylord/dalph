...
barrel from the package root, so that resolved point is omitted below.

## 1. Meaning of “expected visible behavior”

- Choice: Compared journal tags, executor reports, and operation-selection decisions—not the complete trace.
- Limited confidence: The issue does not precisely define which observations are actor-visible.
- Risk: Incorrect claim, tracker, or worktree interactions could pass.
- Verify: Have the scenario owner identify the required visible outputs, then encode those exact fields.

## 2. Fresh-run-only starting facts

- Choice: Cassette starting state permits only `executorWork: "NoPriorReport"` and `journal: "Empty"`.
- Limited confidence: This fits the implemented singleton scenario but not recovery scenarios.
- Risk: The schema may be too narrow for the intended reusable catalog.
- Verify: Try authoring one recovered or interrupted scenario from the parent specification.

## 3. Synthetic recovery activation

- Choice: Added `journaledFreshRunRecoveryActivationLayer`, which always supplies an empty recovered frontier.
- Limited confidence: It exercises production services but bypasses the normal recovery source.
- Risk: A cassette can pass despite broken production recovery composition.
- Verify: Run the same cassette through the actual application composition, or extract a common fake-provider production layer.

## 4. “Complete coordination loop”
...
## 5. Operational state versus complete state
...
## 6. Illegal early-start construction
...
## 7. Alpha-renaming implementation
...
## 8. Which identities may be renamed
...
## 11. Existing stress-test strictness
...
## 13. Size experiment representativeness
...
## 15. Chronology of outside occurrences
...
## 20. Public support seams across package boundaries
...
## 21. GitHub and project-memory follow-through
