...
it("gives coverage and complexity the same canonical explicit-candidate base", async () => {
  ...
  const { invocationArguments, invocationCoverageBases, result } = await runQualityGateFixture({
    environment: { DALPH_COVERAGE_BASE_SHA: execFileSync("git", ["rev-parse", "HEAD^^"], { encoding: "utf8" }).trim() },
